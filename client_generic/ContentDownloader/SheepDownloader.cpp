...
void SheepDownloader::updateCachedSheep()
{
  ...
  if (Shepherd::getClientFlock(&fClientFlock))
  {
    ...
    for (uint32 i = 0; i < fClientFlock.size(); i++)
    {
      ...
      {
        //	If it is than run through the server flock to see if it is still
        // there.
        uint32 j;
        for (j = 0; j < fServerFlock.size(); j++)
        {
          if (fServerFlock[j]->id() == currentSheep->id() &&
              fServerFlock[j]->generation() == currentSheep->generation())
            break;
        }

        //	If it was not found on the server then it is time to delete the
        // file.
                                   ...
                                   const int getGenerationType) const
{
  ...
  return (Shepherd::cacheSize(getGenerationType) &&
  ...
  if (Shepherd::cacheSize(getGenerationType) != 0)
  {
    while (fClientFlock.size())
    {
      ...
      for (uint32 i = 0; i < fClientFlock.size(); i++)
      {
        ...
        uint16 curPlayCount =
            g_PlayCounter().PlayCount(curSheep->generation(), curSheep->id());
      ...
      if (oldest_time && cacheOverflow(total, getGenerationType))
      {
        ...
        temp << "Deleted: " << filename << ", played:" << playcount << " time"
             << ((playcount == 1) ? "," : "s,") << temptime;
...
void SheepDownloader::findSheepToDownload()
{
  ...
  try
  {
    ...
    // if there are at least three sheep to display in content folder, sleep,
    // otherwise start to download immediately
    if (fClientFlock.size() > 3)
    {
      ...
      // timed out frames.
      g_Log->Info("Chilling for %d seconds before trying to download sheeps...",
    ...
    while (1)
    {
      ...
      if (statfs(Shepherd::jsonPath(), &buf) >= 0)
      {
        lpFreeBytesAvailable =
            (boost::uintmax_t)buf.f_bavail * (boost::uintmax_t)buf.f_bsize;
      }
      else
        incorrect_folder = true;
#endif
      ...
      incorrect_folder =
          incorrect_folder || (!isFolderAccessible(Shepherd::jsonPath()) ||
      ...
      else
      {
        ...
        if (getSheepList())
        {
          ...
          do
          {
            ...
            {
              //	Iterate the client flock to see if it allready exists in
              // the cache.
            ...
            for (i = 0; i < fServerFlock.size(); i++)
            {
              ...
              //	If it is not found and the cache is ok to store than
              // check if the file should be downloaded based on rating and
              // server file write time.
              if ((j == fClientFlock.size()) &&
                  !cacheOverflow((double)fServerFlock[i]->fileSize(),
                                 fServerFlock[i]->getGenerationType()))
              {
                ...
                if ((best_ctime == 0 && best_ctime_old == 0) ||
                    (fServerFlock[i]->rating() > best_rating &&
                     fServerFlock[i]->rating() <= best_rating_old) ||
                    (fServerFlock[i]->rating() == best_rating &&
                     fServerFlock[i]->fileWriteTime() < best_ctime))
                {
                  ...
                  // if (useDreamAI)
                  {
                    //  timeCheck = fServerFlock[i]->fileWriteTime() !=
                    //  best_ctime_old;
                  }
                  // else
                  {
                    timeCheck =
                        fServerFlock[i]->fileWriteTime() > best_ctime_old;
                  }
            ...
            if (best_anim != -1)
            {
              ...
              deleteCached(
                  fServerFlock[static_cast<size_t>(best_anim)]->fileSize(),
                  fServerFlock[static_cast<size_t>(best_anim)]
                      ->getGenerationType());

              std::stringstream downloadingsheepstr;
              downloadingsheepstr << "Downloading sheep " << downloadedcount + 1
                                  << "/" << fServerFlock.size() << "...\n";
              Shepherd::setDownloadState(
                  downloadingsheepstr.str() +
                  fServerFlock[static_cast<size_t>(best_anim)]->URL());

              g_Log->Info("Best sheep to download rating=%d, fServerFlock "
                          "index=%d, write time=%s",
                          best_rating, best_anim, ctime(&best_ctime));
              ...
              else
              {
                ...
                best_anim_old_url =
                    fServerFlock[static_cast<size_t>(best_anim_old)]->URL();
              }
            }
            boost::this_thread::interruption_point();
          } while (best_anim != -1);
          ...
          if (best_anim_old == -1)
          {
            ...
            tmp << "All available dreams downloaded, will retry in {"
                << std::fixed << std::setprecision(0) << failureSleepDuration
                << "}...";
          ...
          {
            //	Gradually increase duration betwee 10 seconds and TIMEOUT, if
            ...
            tmp << "Downloading failed, will retry in {" << std::fixed
