...
void Torrent::resetSingleDownload()
{
    if (fTorrentHandle.is_valid())
    {
        fSingleDownload = -1;
...
float Torrent::getRatio(int place)
{
    if (fTorrentHandle.is_valid())
    {
        libtorrent::torrent_status status = fTorrentHandle.status();
...
libtorrent::torrent_status::state_t Torrent::getStatus()
{
    if (fTorrentHandle.is_valid())
