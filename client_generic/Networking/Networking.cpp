...
bool CCurlTransfer::InterruptiblePerform()
{
  ...
  while (1)
  {
    ...
    _code =
        curl_multi_fdset(m_pCurlM, &fd_read, &fd_write, &fd_except, &max_fd);
    ...
    while ((err = select(max_fd + 1, &fd_read, &fd_write, &fd_except, &tval)) <
           0)
    {
#ifndef WIN32
      if (errno != EINTR)
...
bool CCurlTransfer::Perform(const std::string &_url)
{
  ...
  if (!Verify(curl_easy_getinfo(m_pCurl, CURLINFO_RESPONSE_CODE, &m_HttpCode)))
    return false;
  if (m_HttpCode != 200)
  {
                              ...
                              const fp8 _bytesTransferred)
{
  ...
  if (_pTransfer->Status() == "Active")
  {
    ...
    if (_bytesTransferred > 1024 * 1024)
      ...
      tmp << std::fixed << std::setprecision(0) << " (" << _bytesTransferred
          << " B)";
  }
