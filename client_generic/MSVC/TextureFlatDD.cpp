...
CTextureFlatDS::~CTextureFlatDS() { SAFE_RELEASE(m_pTextureDS); }
...
bool CTextureFlatDS::Upload(spCImage _spImage)
{
  ...
  if (!m_pTextureDS)
  {
    ...
    g_Log->Info("%s texture created, HR=%d", format.GetDescription().c_str(),
                hr);
...
bool CTextureFlatDS::Bind(const uint32 _index)
{
  ...
  sd.dwSize = sizeof(sd);
  sd.dwFlags =
      DDSD_WIDTH | DDSD_HEIGHT | DDSD_PITCH | DDSD_LPSURFACE | DDSD_PIXELFORMAT;
...
bool CTextureFlatDS::Unbind(const uint32 _index) { return true; }
