...
class DXDLLFun : public CSingleton<DXDLLFun>
{
  ...
  DXDLLFun()
      : d3dx9_hmodule(NULL), D3DXCompileShader_fun(NULL),
        D3DXGetVertexShaderProfile_fun(NULL), D3DXCreateFontA_fun(NULL),
        D3DXCreateSprite_fun(NULL), D3DXMatrixMultiply_fun(NULL),
  ...
  typedef HRESULT(WINAPI *D3DXCreateSprite_fun_type)(LPDIRECT3DDEVICE9 pDevice,
...
public:
  ...
  bool Init()
  {
    ...
    m_bInitDone = true;
    static const std::string suffix[20] = {
    ...
    D3DXCreateFontA_fun = (D3DXCreateFontA_fun_type)GetProcAddress(
        d3dx9_hmodule, "D3DXCreateFontA");
    if (D3DXCreateFontA_fun == NULL)
    {
      g_Log->Error("Unable to load D3DXCreateFontA_fun");
      return false;
    }
    D3DXCreateSprite_fun = (D3DXCreateSprite_fun_type)GetProcAddress(
        d3dx9_hmodule, "D3DXCreateSprite");
    if (D3DXCreateSprite_fun == NULL)
    {
      g_Log->Error("Unable to load D3DXCreateSprite_fun");
      return false;
    }
    D3DXMatrixMultiply_fun = (D3DXMatrixMultiply_fun_type)GetProcAddress(
        d3dx9_hmodule, "D3DXMatrixMultiply");
    if (D3DXMatrixMultiply_fun == NULL)
    {
      g_Log->Error("Unable to load D3DXMatrixMultiply_fun");
      return false;
    }
    D3DXGetPixelShaderProfile_fun =
        (D3DXGetPixelShaderProfile_fun_type)GetProcAddress(
            d3dx9_hmodule, "D3DXGetPixelShaderProfile");
    if (D3DXGetPixelShaderProfile_fun == NULL)
    {
      g_Log->Error("Unable to load D3DXGetPixelShaderProfile_fun");
      return false;
    }
    D3DXGetVertexShaderProfile_fun =
        (D3DXGetVertexShaderProfile_fun_type)GetProcAddress(
            d3dx9_hmodule, "D3DXGetVertexShaderProfile");
    if (D3DXGetVertexShaderProfile_fun == NULL)
    {
      g_Log->Error("Unable to load D3DXGetVertexShaderProfile_fun");
      return false;
    }
    D3DXCompileShader_fun = (D3DXCompileShader_fun_type)GetProcAddress(
        d3dx9_hmodule, "D3DXCompileShader");
