...
bool CDisplayDD::InitDD(HWND hWnd)
{
  ...
  else
  {
    if (m_bPreview)
    {
      if (m_dd.Create(hWnd, m_ParentWindowHandle) == FALSE ||
          m_dd.CreateSurface(m_Width, m_Height) == FALSE)
      {
        g_Log->Info(m_dd.GetLastErrorString());
        return false;
      }
    }
    else if (m_dd.Create(hWnd, m_ParentWindowHandle) == FALSE ||
             m_dd.CreateSurface(m_Width, m_Height) == FALSE)
    {
      g_Log->Info(m_dd.GetLastErrorString());
      return false;
...
LRESULT CALLBACK CDisplayDD::wndProc(HWND hWnd, UINT msg, WPARAM wParam,
                                     LPARAM lParam)
{
  PAINTSTRUCT ps;
...
bool CDisplayDD::Initialize(HWND _hWnd, bool _bPreview)
{
  ...
  if (_bPreview)
  {
    ...
    m_WindowHandle = CreateWindow(
        ...
        rc.right - rc.left, rc.bottom - rc.top, _hWnd, NULL, hInstance, this);
...
HWND CDisplayDD::Initialize(const uint32 _width, const uint32 _height,
                            const bool _bFullscreen)
{
  ...
  if (!_bFullscreen)
  {
    ...
    MoveWindow(m_WindowHandle, rcWindow.left, rcWindow.top, _width + ptDiff.x,
               _height + ptDiff.y, TRUE);
  }
...
void CDisplayDD::Update()
{
  ...
  //	InvalidateRect( m_WindowHandle, NULL, FALSE );    // Invalidate the hwnd
  // so it gets drawn 	UpdateWindow( m_WindowHandle );
  // }
