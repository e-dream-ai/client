...
void electricsheepguiMyDialog2::SaveSettings()
{
    ...
    g_Settings()->Set("settings.app.attributionpng",
                      m_checkAttributionPNG->GetValue());
    ...
    g_Settings()->Set("settings.player.PlaybackMixingMode",
                      m_choicePlaybackMixingMode->GetSelection());
    g_Settings()->Set("settings.player.SeamlessPlayback",
                      m_SeamlessPlayback->GetValue());
    ...
    g_Settings()->Set("settings.player.force_windowed_directx",
                      m_ForceWindowedDX);

    g_Settings()->Set("settings.content.unlimited_cache",
                      m_checkUnlimitedCache->IsChecked());

    g_Settings()->Set("settings.content.unlimited_cache_gold",
                      m_checkGoldUnlimitedCache->IsChecked());

    g_Settings()->Set("settings.content.download_mode",
                      m_checkHttp->GetValue());
    g_Settings()->Set("settings.generator.enabled",
                      m_checkRenderFrames->GetValue());
    g_Settings()->Set("settings.generator.all_cores",
                      m_checkMulticore->GetValue());
    g_Settings()->Set("settings.generator.save_frames",
                      m_checkKeepFrames->GetValue());

    g_Settings()->Set("settings.content.negvotedeletes",
                      m_checkNegVoteDeletes->GetValue());
    g_Settings()->Set("settings.player.vbl_sync", m_VerticalSync->GetValue());
    g_Settings()->Set("settings.player.reversedisplays",
                      m_ReverseDisplays->GetValue());

    g_Settings()->Set("settings.generator.nickname",
                      std::string(m_textDrupalName->GetValue()));

    if (g_Settings()->Get("settings.content.password_md5", std::string("")) !=
        "")
        g_Settings()->Set("settings.content.password",
                          g_Settings()->Get("settings.content.password_md5",
                                            std::string("")));

    g_Settings()->Set("settings.player.DisplayMode",
                      m_radioDisplayMode->GetSelection());
    g_Settings()->Set("settings.player.MultiDisplayMode",
                      m_radioMultiDisplayMode->GetSelection());

    g_Settings()->Set("settings.content.sheepdir",
                      std::string(m_dirContent->GetPath()));
                ...
                if (itr->path().extension() == std::string(".avi"))
                {
                                    ...
                                    &generation, &id, &first, &last))
                    {
                            ...
                            (generation < 10000 && sheeptype == 0))
                        {
                            ...
                            if (stat((mpegpath + wxT("/") + fname).c_str(),
                                     &sbuf) == 0)
                                retval += sbuf.st_size;
                        }
                    }
...
void electricsheepguiMyDialog2::LoadSettings()
{
    ...
    m_checkAttributionPNG->SetValue(
        g_Settings()->Get("settings.app.attributionpng", true));
    m_spinCache->SetValue(wxString::Format(
        wxT("%d"), g_Settings()->Get("settings.content.cache_size", 2000)));
    m_spinGoldCache->SetValue(wxString::Format(
        wxT("%d"),
        g_Settings()->Get("settings.content.cache_size_gold", 2000)));
    m_spinDecodeFps->SetValue(wxString::Format(
    ...
    m_ForceWindowedDX =
        g_Settings()->Get("settings.player.force_windowed_directx", true);
    ...
    if (g_Settings()->Get("settings.content.unlimited_cache_gold", false) ==
        true)
    {
    ...
    m_checkHttp->SetValue(
        g_Settings()->Get("settings.content.download_mode", true));
    m_checkRenderFrames->SetValue(
        g_Settings()->Get("settings.generator.enabled", true));
    m_checkMulticore->SetValue(
        g_Settings()->Get("settings.generator.all_cores", false));
    m_choicePlaybackMixingMode->SetSelection(
        g_Settings()->Get("settings.player.PlaybackMixingMode", 0));
    m_SeamlessPlayback->SetValue(
        g_Settings()->Get("settings.player.SeamlessPlayback", false));
    m_QuietMode->SetValue(
        g_Settings()->Get("settings.player.quiet_mode", true));
    m_DirectDraw->SetValue(
        g_Settings()->Get("settings.player.directdraw", false));
    m_checkKeepFrames->SetValue(
        g_Settings()->Get("settings.generator.save_frames", false));
    m_checkNegVoteDeletes->SetValue(
        g_Settings()->Get("settings.content.negvotedeletes", true));
    m_VerticalSync->SetValue(
        g_Settings()->Get("settings.player.vbl_sync", false));
    m_ReverseDisplays->SetValue(
        g_Settings()->Get("settings.player.reversedisplays", false));

    m_textDrupalName->ChangeValue(
        g_Settings()->Get("settings.generator.nickname", std::string("")));
    if (m_textDrupalName->GetValue() != wxEmptyString)
        m_textDrupalPassword->ChangeValue(g_Settings()->Get(
            "settings.content.password_md5", std::string("")));
    ...
    m_radioDisplayMode->SetSelection(
        g_Settings()->Get("settings.player.DisplayMode", 2));
    m_radioMultiDisplayMode->SetSelection(
        g_Settings()->Get("settings.player.MultiDisplayMode", 0));

    m_textProxyHost->SetValue(
        g_Settings()->Get("settings.content.proxy", std::string("")));
    m_textProxyUser->SetValue(
        g_Settings()->Get("settings.content.proxy_username", std::string("")));
    m_textProxyPassword->SetValue(
...
#ifdef WIN32
    wxString tmpstr = g_Settings()->Get("settings.content.sheepdir",
                                        std::string(szPath) + "content");
    while (tmpstr.size() > 1 && (tmpstr.at(tmpstr.size() - 1) == '/' ||
                                 tmpstr.at(tmpstr.size() - 1) == '\\'))
        tmpstr.erase(tmpstr.size() - 1);
    m_dirContent->SetPath(tmpstr);
#else
    m_dirContent->SetPath(g_Settings()->Get("settings.content.sheepdir",
                                            std::string(szPath) + "content"));
#endif
    wxString newlabel = wxT(
        "New sheep are created everyday.  When the screensaver runs,\nit tries "
        "to download them, and saves them on your hard disk,\ndeleting old "
        "ones "
        "to make room.  Login to get more sheep.\nIt is currently using ");
    wxString newlabelgold = wxT("It is currently using ");

#ifdef WIN32
    int freeflocksizembs =
        (int)GetFlockSizeMBs(m_dirContent->GetPath() + "\\mpeg", 0);
    int goldflocksizembs =
        (int)GetFlockSizeMBs(m_dirContent->GetPath() + "\\mpeg", 1);
#else
    int freeflocksizembs =
        (int)GetFlockSizeMBs(m_dirContent->GetPath() + "/mpeg", 0);
    int goldflocksizembs =
    ...
    newlabel +=
        wxString::Format(wxT("%d %s %d%s"), freeflocksizembs, wxT("MB (of"),
                         totalflocksizembs, wxT("MB total)."));
    newlabelgold +=
        wxString::Format(wxT("%d %s %d%s"), goldflocksizembs, wxT("MB (of"),
                         totalflocksizembs, wxT("MB total)."));
...
void electricsheepguiMyDialog2::Login()
{
    ...
    std::string nickencoded = Encode(
        g_Settings()->Get("settings.generator.nickname", std::string("")));
    std::string passencoded = Encode(
        g_Settings()->Get("settings.content.password_md5", std::string("")));

    if (nickencoded == std::string("") || passencoded == std::string(""))
    {
        wxMutexGuiEnter();
        FireLoginStatusUpdateEvent("");
        FireLoginInfoUpdateEvent(
            "Create an account on the server and upgrade to Gold for higher "
    ...
    curl_easy_setopt(
        pCurl, CURLOPT_USERPWD,
        std::string(nickencoded + std::string(":") + passencoded).c_str());
    ...
    snprintf(url, 1024 * 5, "http://%s/query.php?q=redir&u=%s&p=%s&v=%s&i=%s",
             REDIRECT_SERVER, nickencoded.c_str(), passencoded.c_str(),
             CLIENT_VERSION, m_UniqueId.c_str());
            ...
            if (code == 200)
            {
                ...
                if (doc.Parse(m_Response.c_str(), NULL, TIXML_ENCODING_UTF8))
                {
                        ...
                        *role != 0)
                    {
                        ...
                        {
                            FireLoginInfoUpdateEvent(
                                "Create an account on the server and upgrade "
                                "to Gold for "
                                "higher \nresolution and other benefits.\n");
                        }
                        else if (m_Role == "registered")
                        {
                            FireLoginInfoUpdateEvent(
                                "Thank you for registering, you may become a "
                                "member for "
                                "access to\nour private server with more "
                                "sheep, higher "
                                "resolution sheep,\nand other interactive "
                                "features.");
                        }
                        else if (m_Role == "member")
                        {
                            FireLoginInfoUpdateEvent(
                                "Create an account on the server and upgrade "
                                "to Gold for "
                        ...
                        {
                            FireLoginInfoUpdateEvent(
                                "Thank you for your membership. Please access "
                                "the server to "
                                "manage \nyour account.");
                        }
                        ...
                        {
                            FireLoginStatusUpdateEvent(
                                "Logged in (role: " + m_Role + ").");

                            g_Settings()->Set("settings.content.registered",
                                              true);
                            wxMutexGuiLeave();
    ...
    wxMutexGuiEnter();
    if (m_Role == "error")
        FireLoginStatusUpdateEvent("Not logged in.");
    else
        FireLoginStatusUpdateEvent("Logged in but not gold");
    wxMutexGuiLeave();
...
{
    std::string path = g_Settings()->Get("settings.content.sheepdir",
    ...
    : MyDialog2(parent)
{
    ...
    TupleStorage::IStorageInterface::CreateFullDirectory(std::string(szPath) +
                                                         "Logs/");
    ...
    m_AboutText->WriteText(wxT(
        " is a collaborative abstract artwork founded by Scott Draves. The "
        "thousands of computers it runs on work together as a supercomputer to "
        "render the animations (each frame takes about an hour). Everyone "
        "watching can influence what everyone sees by voting, and the more "
        "popular sheep mate with each other and reproduce according to a "
        "genetic "
        "algorithm, hence the flock evolves to satisfy human desire.  You can "
        "also design your own sheep and upload them into the gene pool."));
    m_AboutText->LineBreak();
    m_AboutText->LineBreak();

    m_AboutText->AppendText(
        wxT("You can buy high-quality limited edition artworks made with the "
            "Electric Sheep, as well as Blu-Rays, DVDs and T-shirts, from "));
    ...
    m_AboutText->WriteText(wxT(
        ". Proceeds from these sales keep the network free to the public."));
    m_AboutText->LineBreak();
    m_AboutText->LineBreak();
...
void electricsheepguiMyDialog2::OnDrupalPasswordTextEnter(wxCommandEvent &event)
{
    ...
    g_Settings()->Set("settings.content.registered", false);
    g_Settings()->Set("settings.content.password",
                      std::string(m_textDrupalPassword->GetValue()));
    g_Settings()->Set("settings.generator.nickname",
                      std::string(m_textDrupalName->GetValue()));
    g_Settings()->Set(
        "settings.content.password_md5",
        computeMD5(
...
void electricsheepguiMyDialog2::OnCreateClick(wxCommandEvent &event)
{
    ...
    browserlink << "http://electricsheep.org/account/" << m_Role
                << "?u=" << nickencoded << "&p=" << passencoded;
...
void electricsheepguiMyDialog2::OnContentDirChanged(wxFileDirPickerEvent &event)
{
    ...
    if (IsUserAnAdmin())
    {
        wxExecute(wxString(L"SETACL.EXE -on \"") + contentdir +
                  wxString(L"\" -ot file -actn ace -ace "
                           L"\"n:S-1-1-0;p:full,write_dacl;s:y;\""));
    }
    else
    {
        wxMessageBox(wxT("This action requires administrator rights"),
                     wxT("Program is not currently running as admin"),
                     wxICON_INFORMATION);
        wxExecute(wxString(L"SETACL.EXE -on \"") + contentdir +
                  wxString(L"\" -ot file -actn ace -ace "
                           L"\"n:S-1-1-0;p:full,write_dacl;s:y;\""));
...
void electricsheepguiMyDialog2::OnOpenClick(wxCommandEvent &event)
{
    ...
    if (iReturn == -1)
        wxExecute(L"dolphin " + m_dirContent->GetPath());
#endif
}
