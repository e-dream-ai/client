    ...
    for (directory_iterator itr(p); itr != end_itr; ++itr)
    {
      if (!is_directory(itr->status()))
      {
        ...
        if (itr->path().extension() == std::string(".avi"))
        {
          ...
          if (4 == sscanf(fname.c_str(), "%d=%d=%d=%d.avi", &generation, &id,
                          &first, &last))
          {
...
void electricsheepguiMyDialog2::LoadSettings()
{
  ...
  m_spinGoldCache->SetValue(wxString::Format(
      wxT("%d"), g_Settings()->Get("settings.content.cache_size_gold", 2000)));
  ...
  if (m_textDrupalName->GetValue() != wxEmptyString)
    m_textDrupalPassword->ChangeValue(
        g_Settings()->Get("settings.content.password_md5", std::string("")));
...
void electricsheepguiMyDialog2::Login()
{
  ...
  std::string nickencoded =
      Encode(g_Settings()->Get("settings.generator.nickname", std::string("")));
      ...
      if (code == 200)
      {
        ...
        if (doc.Parse(m_Response.c_str(), NULL, TIXML_ENCODING_UTF8))
        {
          ...
          if (host != NULL && *host != 0 && role != NULL && *role != 0)
          {
            ...
            {
              FireLoginInfoUpdateEvent(
                  "Create an account on the server and upgrade to Gold for "
                  "higher \nresolution and other benefits.\n");
            }
            else if (m_Role == "registered")
            {
              FireLoginInfoUpdateEvent(
                  "Thank you for registering, you may become a member for "
                  "access to\nour private server with more sheep, higher "
            ...
            {
              FireLoginInfoUpdateEvent(
                  "Thank you for your membership. Please access the server to "
                  "manage \nyour account.");
            }
    ...
    : MyDialog2(parent)
{
  ...
  m_AboutText->WriteText(wxT(
      ...
      "popular sheep mate with each other and reproduce according to a genetic "
  ...
  m_AboutText->WriteText(
      wxT(". Proceeds from these sales keep the network free to the public."));
  m_AboutText->LineBreak();
  m_AboutText->LineBreak();
