...
class MyDialog2 : public wxDialog
{
  ...
  protected:
    ...
    virtual void OnDrupalPasswordTextEnter(wxCommandEvent& event)
    {
        event.Skip();
    }
    virtual void OnTestAccountButtonClick(wxCommandEvent& event)
    {
        event.Skip();
    }
    ...
    virtual void OnGoldUnlimitedCacheCheck(wxCommandEvent& event)
    {
        event.Skip();
    }
    ...
    virtual void OnContentDirChanged(wxFileDirPickerEvent& event)
    {
        event.Skip();
    }
  ...
  public:
    MyDialog2(wxWindow* parent, wxWindowID id = wxID_ANY,
              const wxString& title = wxT("Electric Sheep Settings"),
              const wxPoint& pos = wxDefaultPosition,
              const wxSize& size = wxSize(400, 512),
