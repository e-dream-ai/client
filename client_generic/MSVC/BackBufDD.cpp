...
BOOL CBackBufDD::Create(HWND Main, HWND View, GUID *Driver, bool Exclusive)
{
  ...
  if (FAILED(m_hr = DirectDrawCreateEx(Driver, (VOID **)&m_dd, IID_IDirectDraw7,
                                       NULL)))
  {
        ...
        { // mirror to intermediate surface
          ...
          if (hRet == DD_OK)
          {
            ...
            if (hRet == DD_OK)
            {
              ...
              r.bottom += h;
              hRet =
                  m_Mirror->Blt(&r, m_Back, NULL, WAITFX, &fx); // lower right
              if (hRet == DD_OK)
              {
                fx.dwDDFX = DDBLTFX_MIRRORUPDOWN;
                r.left -= w;
                r.right -= w;
                hRet =
                    m_Mirror->Blt(&r, m_Back, NULL, WAITFX, &fx); // lower left
                if (hRet == DD_OK)
          ...
          if (hRet == DD_OK)
          {
            ...
            if (hRet == DD_OK)
            {
              ...
              if (hRet == DD_OK)
              {
                ...
                r.right -= w;
                hRet =
                    m_Front->Blt(&r, m_Back, NULL, WAITFX, &fx); // lower left
              }
            }
          }
        }
    ...
    if (m_DrawBuf->GetDC(&hdc) == DD_OK)
    {
      while (m_Tex.size() > 0)
      {
        ...
        if (curtex->GetDC(&sdc) == DD_OK)
        {
          ...
          StretchBlt(hdc, int(_rect.m_X0 * m_Width), int(_rect.m_Y0 * m_Height),
                     int((_rect.m_X1 - _rect.m_X0) * m_Width),
                     int((_rect.m_Y1 - _rect.m_Y0) * m_Height), sdc, 0, 0,
                     curwidth, curheight, SRCCOPY);
