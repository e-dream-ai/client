...
/*
        ...
        09      14oct05	kill DirectDraw's leaky timer
                10		19oct05	in SetExclusive, save/restore window
   placement 11		26oct05	don't set position before restoring placement 12
   05may06	add GetErrorString
...
bool CBackBufDD::CreateSurface(int Width, int Height)
{
    ...
    else
    {
        ...
        if (m_IsMirrored)
        {
            ...
            {
                // OutputDebugStringA("m_dd->CreateSurface(&sd, &m_Back, NULL)
                // 2");
                return (FALSE);
            }
            if (m_Options & OPT_MIRROR_PRECISE)
            { // mirror to intermediate surface
                sd.ddsCaps.dwCaps = DDSCAPS_OFFSCREENPLAIN | DDSCAPS_3DDEVICE |
                                    DDSCAPS_VIDEOMEMORY;
                sd.dwWidth = Width;
                sd.dwHeight = Height;
                if (FAILED(m_hr = m_dd->CreateSurface(&sd, &m_Mirror, NULL)))
                {
                    // OutputDebugStringA("m_dd->CreateSurface(&sd, &m_Mirror,
                    // NULL) 3");
                    return (FALSE);
                }
            }
    ...
    while (1)
    {
        ...
        if (m_IsExclusive)
        {
            ...
            { // mirrored blit
                ...
                if (hRet == DD_OK)
                {
                    ...
                    r.left += w;
                    r.right += w;
                    hRet = m_Back->Blt(&r, m_Mirror, NULL, WAITFX,
                                       &fx); // upper right
                    if (hRet == DD_OK)
                    {
                        fx.dwDDFX =
                            DDBLTFX_MIRRORUPDOWN | DDBLTFX_MIRRORLEFTRIGHT;
                        r.bottom += h;
                        r.top += h;
                        hRet = m_Back->Blt(&r, m_Mirror, NULL, WAITFX,
                                           &fx); // lower right
                        if (hRet == DD_OK)
                        {
                            ...
                            if (hRet == DD_OK)
                                hRet = m_Front->Flip(
                                    NULL, DDFLIP_WAIT); // update display
                        }
                    }
                }
            }
        ...
        else
        {
            ...
            { // mirrored blit
                ...
                if (m_Mirror != NULL)
                { // mirror to intermediate surface
                    hRet = m_Mirror->Blt(&r, m_Back, NULL, WAIT,
                                         NULL); // upper left
                    if (hRet == DD_OK)
                    {
                        fx.dwDDFX = DDBLTFX_MIRRORLEFTRIGHT;
                        r.left += w;
                        r.right += w;
                        hRet = m_Mirror->Blt(&r, m_Back, NULL, WAITFX,
                                             &fx); // upper right
                        if (hRet == DD_OK)
                        {
                            fx.dwDDFX =
                            ...
                            if (hRet == DD_OK)
                            {
                                ...
                                hRet = m_Mirror->Blt(&r, m_Back, NULL, WAITFX,
                                                     &fx); // lower left
                                if (hRet == DD_OK)
                                { // blit mirrored image to display
                                    RECT wr = {0, 0, m_Width, m_Height};
                                    wr.left += org.x;
                                    wr.right += org.x;
                                    wr.top += org.y;
                                    wr.bottom += org.y;
                                    hRet = m_Front->Blt(&wr, m_Mirror, NULL,
                                                        WAIT, NULL);
                                }
                            }
                        }
                ...
                { // mirror directly to display; quick and dirty
                    r.left += org.x;
                    r.right += org.x;
                    r.top += org.y;
                    r.bottom += org.y;
                    hRet = m_Front->Blt(&r, m_Back, NULL, WAIT,
                                        NULL); // upper left
                    if (hRet == DD_OK)
                    {
                        fx.dwDDFX = DDBLTFX_MIRRORLEFTRIGHT;
                        r.left += w;
                        r.right += w;
                        hRet = m_Front->Blt(&r, m_Back, NULL, WAITFX,
                                            &fx); // upper right
                        if (hRet == DD_OK)
                        {
                            fx.dwDDFX =
...
void CBackBufDD::SetOptions(int Options) { m_Options = Options; }
...
bool CBackBufDD::SetExclusive(HWND Main, HWND View, bool Enable)
{
    ...
    if (Enable)
    {
        SetWindowPos(Main, HWND_TOPMOST, // set topmost attribute
                     ...
                     0); // and go full-screen
    }
    else
    {
        SetWindowPos(Main, HWND_NOTOPMOST, // clear topmost attribute
                     0, 0, 0, 0,
                     SWP_NOMOVE | SWP_NOSIZE); // but don't set position
...
bool CBackBufDD::GetMonitorGUID(HMONITOR hMon, GUID& MonGuid)
{
    ...
    lpDDEnumEx =
        (LPDIRECTDRAWENUMERATEEXW)GetProcAddress(h, "DirectDrawEnumerateExW");
    bool retc = FALSE;
    if (lpDDEnumEx)
    { // if the function was imported
        MONGUID mg = {0};
        mg.Mon = hMon; // tell callback which monitor to look for
        lpDDEnumEx(DDEnumCallbackEx, &mg, DDENUM_ATTACHEDSECONDARYDEVICES);
        if (mg.Valid)
        {                      // if callback found the monitor
        ...
        if (m_DrawBuf->GetDC(&hdc) == DD_OK)
        {
            while (m_Tex.size() > 0)
            {
                ...
                if (curtex->GetDC(&sdc) == DD_OK)
                {
                    ...
                    StretchBlt(hdc, int(_rect.m_X0 * m_Width),
                               int(_rect.m_Y0 * m_Height),
                               int((_rect.m_X1 - _rect.m_X0) * m_Width),
                               int((_rect.m_Y1 - _rect.m_Y0) * m_Height), sdc,
                               0, 0, curwidth, curheight, SRCCOPY);
...
void CBackBufDD::SetContexts(LPDIRECTDRAWSURFACE7 _tex, int _SrcWidth,
                             int _SrcHeight)
{
    if (!IsCreated())
        return;
...
HRESULT CBackBufDD::GetLastError() const { return (m_hr); }
...
bool CBackBufDD::IsCreated() const { return (m_dd != NULL); }

bool CBackBufDD::IsMirrored() const { return (m_IsMirrored); }

int CBackBufDD::GetOptions() const { return (m_Options); }
...
bool CBackBufDD::IsExclusive() const { return (m_IsExclusive); }
