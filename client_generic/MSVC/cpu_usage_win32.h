...
class ESCpuUsage
{
  ...
  public:
    ...
    bool GetCpuUsage(int &_total, int &_es)
    {
        ...
        {
            usr.LowPart =
                userTime.dwLowDateTime - m_LastESUserTime.dwLowDateTime;
            usr.HighPart =
                userTime.dwHighDateTime - m_LastESUserTime.dwHighDateTime;
            ker.LowPart =
                kernelTime.dwLowDateTime - m_LastESKernelTime.dwLowDateTime;
            ker.HighPart =
            ...
            if (period > 0.)
            {
                _es = int((ker.QuadPart + usr.QuadPart) * 100. /
                          (period * 1e+7) / sysinfo.dwNumberOfProcessors);
            }
            else
                _es = 0;
        ...
        if (GetSystemTimes(&idleTime, &kernelTime, &userTime) != 0)
        {
            ...
            usr.HighPart =
                userTime.dwHighDateTime - m_LastUserTime.dwHighDateTime;
            ker.LowPart =
                kernelTime.dwLowDateTime - m_LastKernelTime.dwLowDateTime;
            ker.HighPart =
                kernelTime.dwHighDateTime - m_LastKernelTime.dwHighDateTime;
            idl.LowPart = idleTime.dwLowDateTime - m_LastIdleTime.dwLowDateTime;
            idl.HighPart =
                idleTime.dwHighDateTime - m_LastIdleTime.dwHighDateTime;

            if (ker.QuadPart + usr.QuadPart != 0)
            {
                _total = int((ker.QuadPart + usr.QuadPart - idl.QuadPart) *
                             100 / (ker.QuadPart + usr.QuadPart));
            }
            else
                _total = 0;
