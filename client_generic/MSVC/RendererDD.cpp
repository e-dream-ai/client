...
void CRendererDD::Defaults() { g_Log->Info("CRendererDD::Defaults()"); }

spCTextureFlat CRendererDD::NewTextureFlat(spCImage _spImage,
                                           const uint32 _flags)
{
                                                ...
                                                const std::string& _text)
{
    ...
    if (hOldFont = (HFONT)SelectObject(xdc, pDDFont))
    {
        ...
        int32 h = DrawTextA(xdc, ".", -1, &dotRect,
                            DT_LEFT | DT_WORDBREAK | DT_NOCLIP | DT_CALCRECT);
        int32 dotWidth = dotRect.right - dotRect.left;

        RECT rect = {0, 0, LONG(dispWidth), LONG(dispHeight)};
        h = DrawTextA(xdc, pTmp, -1, &rect,
                      DT_LEFT | DT_WORDBREAK | DT_NOCLIP | DT_CALCRECT);
                       ...
                       const Base::Math::CRect& _rect, uint32 _flags)
{
    ...
    // COLORREF ddColor = D3DCOLOR_COLORVALUE( _color.m_X, _color.m_Y,
