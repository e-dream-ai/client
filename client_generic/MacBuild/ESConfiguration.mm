...
- (void)fixFlockSize
{
  ...
  if (mp4path != NULL && *mp4path)
  {
    ...
    [str
        replaceOccurrencesOfString:@"^1"
                        withString:[NSString stringWithFormat:@"%ld", flockSize]
                           options:0
...
- (void)loadSettings
{
  ...
  [synchronizeVBL
      setState:ESScreensaver_GetBoolSetting("settings.player.vbl_sync", false)];
  ...
  [useProxy setState:ESScreensaver_GetBoolSetting("settings.content.use_proxy",
                                                  false)];
  ...
  [proxyPassword setStringValue:(__bridge_transfer NSString *)
                                    ESScreensaver_CopyGetStringSetting(
...
- (IBAction)chooseContentFolder:(id)__unused sender
{
  ...
  if (field)
  {
    ...
    [openPanel
        setDirectoryURL:[NSURL fileURLWithPath:[path stringByStandardizingPath]
                                   isDirectory:YES]];
