    ...
    while (EnumDisplayDevices(NULL, iDevice, (DISPLAY_DEVICE*)&dispdev, 0))
    {
        ...
        if ((dispdev.StateFlags & DISPLAY_DEVICE_MIRRORING_DRIVER) == 0)
        {
                                   ...
                                   (DISPLAY_DEVICE*)&dispdev2, 0) != 0)
            {
                ...
                {
                    if (EnumDisplaySettings(dispdev.DeviceName,
                                            ...
                                            &devmode) != 0)
                    {
                        ...
                        //{
                        //	For some reason devmode.dmPosition is not always
                        //(0, 0) for the
                        // primary display, so force it.
                        ...
                        pMonitorInfoNew->rcScreen.right =
                            pMonitorInfoNew->rcScreen.left +
                            devmode.dmPelsWidth;
                        pMonitorInfoNew->rcScreen.bottom =
                            pMonitorInfoNew->rcScreen.top +
                            devmode.dmPelsHeight;

                        pMonitorInfoNew->hMonitor = MonitorFromRect(
                            &pMonitorInfoNew->rcScreen, MONITOR_DEFAULTTONULL);
                        g_Log->Info("EnumMonitors %d x=%u y=%u w=%u h=%u",
                                    m_dwNumMonitors, devmode.dmPosition.x,
                                    devmode.dmPosition.y, devmode.dmPelsWidth,
                                    devmode.dmPelsHeight);
                    }
                }
                                     ...
                                     HINSTANCE hInstance)
{
    if (m_BlankUnused)
    {
        ...
        for (DWORD iMonitor = 0; iMonitor < m_dwNumMonitors; iMonitor++)
        {
            ...
            {
                g_Log->Info(
                    "Monitor iMonitor != m_DesiredScreenID %d!=%d x1=%d y1=%d "
                    "x2=%d y2=%d ",
        ...
        for (DWORD iMonitor = 0; iMonitor < m_dwNumMonitors; iMonitor++)
        {
            ...
            g_Log->Info(
                "Monitor %d x1=%d y1=%d x2=%d y2=%d", iMonitor,
            ...
            if (iMonitor != m_DesiredScreenID)
            {
                ...
                SetWindowPos(
                    pMonitorInfo->hWnd, HWND_TOPMOST,
                    monitorInfo.rcMonitor.left, monitorInfo.rcMonitor.top,
                    monitorInfo.rcMonitor.right - monitorInfo.rcMonitor.left,
                    monitorInfo.rcMonitor.bottom - monitorInfo.rcMonitor.top,
                    SWP_NOACTIVATE);
    ...
    for (DWORD iMonitor = 0; iMonitor < m_dwNumMonitors; iMonitor++)
    {
        if (iMonitor == m_DesiredScreenID)
        {
            ...
            if (GetMonitorInfo(pMonitorInfo->hMonitor, &monitorInfo) != 0)
                for (UINT iAdapter = 0;
                     iAdapter < m_pDirect3DInstance->GetAdapterCount();
                     ++iAdapter)
                {
...
bool CDisplayDX::InitDX9()
{
    ...
    if (g_Settings()->Get("settings.player.vbl_sync", false) == true)
        ...
        m_PresentationParams.PresentationInterval =
    ...
    HRESULT devresult = m_pDirect3DInstance->CreateDevice(
        m_bFullScreen ? GetAdapterOrdinal() : D3DADAPTER_DEFAULT,
        D3DDEVTYPE_HAL,
       ...
       ++adapterid)
            {
                    ...
                    m_pDirect3D->GetAdapterIdentifier(adapterid, 0,
       &adapterid9); if (strstr(adapterid9.Description, "PerfHUD") != 0) break;
            }
...
HWND CDisplayDX::createwindow(uint32 _w, uint32 _h, const bool _bFullscreen)
{
    ...
    int yy = 0;
    g_Log->Info(
        "CDisplayDX::createwindow AdjustWindowRectEx x=%u y=%u w=%u h=%u", 0, 0,
        ww, hh);
    MONITORINFO monitorInfo;
...
bool CDisplayDX::Initialize(HWND _hWnd, bool _bPreview)
{
    ...
    if (_bPreview)
    {
        ...
        m_WindowHandle =
            CreateWindow(L"ElectricsheepWndClass", L"Preview", dwStyle, rc.left,
                         rc.top, rc.right - rc.left, rc.bottom - rc.top, _hWnd,
                         NULL, hInstance, NULL);
        g_Log->Info("CDisplayDX::Initialize x=%u y=%u w=%u h=%u", rc.left,
    ...
    else
    {
        ...
        m_WindowHandle =
            _hWnd; // CreateWindowEx( exstyle, "Electricsheep", "Saver", style,
                   // 0, 0, cx, cy, _hWnd, NULL, hInstance, NULL );
        if (m_WindowHandle == NULL)
        {
            g_Log->Error(
                "CDisplayDX::Initialize unable to create window from _hWnd");
            return false;
        }
