...
*/
CDisplayDX::~CDisplayDX() { g_Log->Info("~CDisplayDX()"); }
...
void CDisplayDX::EnumMonitors()
{
  ...
  while (EnumDisplayDevices(NULL, iDevice, (DISPLAY_DEVICE *)&dispdev, 0))
  {
    ...
    if ((dispdev.StateFlags & DISPLAY_DEVICE_MIRRORING_DRIVER) == 0)
    {
      ...
      // EnumDisplayDevices a second time, 	passing dispdev.DeviceName (from
      // the first call) as the first parameter.
      if (EnumDisplayDevices(dispdev.DeviceName, 0, (DISPLAY_DEVICE *)&dispdev2,
                             0) != 0)
      {
        ...
        if (dispdev.StateFlags & DISPLAY_DEVICE_ATTACHED_TO_DESKTOP)
        {
                                  ...
                                  &devmode) != 0)
          {
            ...
            //{
            //	For some reason devmode.dmPosition is not always (0, 0) for the
            // primary display, so force it. pMonitorInfoNew->rcScreen.left = 0;
            ...
            g_Log->Info("EnumMonitors %d x=%u y=%u w=%u h=%u", m_dwNumMonitors,
                                     ...
                                     HINSTANCE hInstance)
{
  if (m_BlankUnused)
  {
    ...
    for (DWORD iMonitor = 0; iMonitor < m_dwNumMonitors; iMonitor++)
    {
      ...
      {
        g_Log->Info("Monitor iMonitor != m_DesiredScreenID %d!=%d x1=%d y1=%d "
                    "x2=%d y2=%d ",
                    iMonitor, m_DesiredScreenID, rc.left, rc.top,
                    rc.right - rc.left, rc.bottom - rc.top);
        pMonitorInfo->hWnd =
            CreateWindowEx(WS_EX_TOPMOST, L"ElectricsheepWndClass", L"ES",
    ...
    for (DWORD iMonitor = 0; iMonitor < m_dwNumMonitors; iMonitor++)
    {
      ...
      g_Log->Info("Monitor %d x1=%d y1=%d x2=%d y2=%d", iMonitor,
...
*/
LRESULT CALLBACK CDisplayDX::wndProc(HWND hWnd, UINT msg, WPARAM wParam,
                                     LPARAM lParam)
{
  PAINTSTRUCT ps;
  ...
  {
  case WM_USER:
    //	All initialization messages have gone through.  Allow 500ms of idle
...
UINT CDisplayDX::GetAdapterOrdinal()
{
  if (m_pDirect3DInstance == NULL ||
      (g_Settings()->Get("settings.player.force_default_adapter", false) ==
       true))
  {
  ...
  for (DWORD iMonitor = 0; iMonitor < m_dwNumMonitors; iMonitor++)
  {
    if (iMonitor == m_DesiredScreenID)
    {
      ...
      if (GetMonitorInfo(pMonitorInfo->hMonitor, &monitorInfo) != 0)
        for (UINT iAdapter = 0;
             iAdapter < m_pDirect3DInstance->GetAdapterCount(); ++iAdapter)
        {
          if (m_pDirect3DInstance->GetAdapterMonitor(iAdapter) ==
              pMonitorInfo->hMonitor)
          {
            g_Log->Info("Using adapter %d for screen %d", iAdapter,
                        m_DesiredScreenID);
            return iAdapter;
          }
        }
...
bool CDisplayDX::InitDX9()
{
  ...
  if (g_Settings()->Get("settings.player.MultiDisplayMode", 0) != 2 &&
      g_Settings()->Get("settings.player.force_windowed_directx", true) == true)
    m_PresentationParams.Windowed = TRUE;
  else
    m_PresentationParams.Windowed = !m_bFullScreen;

  D3DDISPLAYMODE dm = {0};
  if (m_bFullScreen && m_pDirect3DInstance->GetAdapterDisplayMode(
                           GetAdapterOrdinal(), &dm) == D3D_OK)
  {
  ...
  g_Log->Info("Using backbuffer w=%d h=%d",
              m_PresentationParams.BackBufferWidth,
              m_PresentationParams.BackBufferHeight);
  ...
  HRESULT devresult = m_pDirect3DInstance->CreateDevice(
      m_bFullScreen ? GetAdapterOrdinal() : D3DADAPTER_DEFAULT, D3DDEVTYPE_HAL,
      (gl_hFocusWindow == NULL) ? m_WindowHandle : gl_hFocusWindow,
      D3DCREATE_FPU_PRESERVE | dwVertexProcessing, &m_PresentationParams,
      &m_pDevice);
  /*	UINT adapterid = 0;
          for (adapterid = 0; adapterid < m_pDirect3D->GetAdapterCount();
     ++adapterid)
          {
          ...
          HRESULT devresult = m_pDirect3D->CreateDevice( adapterid,
     D3DDEVTYPE_REF, m_WindowHandle, D3DCREATE_FPU_PRESERVE |
     dwVertexProcessing, &m_PresentationParams, &m_pDevice);*/
  if (devresult != D3D_OK)
  {
...
HWND CDisplayDX::createwindow(uint32 _w, uint32 _h, const bool _bFullscreen)
{
  ...
  DISP_CHANGE_SUCCESSFUL ) OutputDebugString("Failed to change display
  settings");
  }*/
  ...
  int xx = 0;
  int yy = 0;
  g_Log->Info("CDisplayDX::createwindow AdjustWindowRectEx x=%u y=%u w=%u h=%u",
              0, 0, ww, hh);
  MONITORINFO monitorInfo;
  ...
  for (DWORD iMonitor = 0; iMonitor < m_dwNumMonitors; iMonitor++)
  {
    if (iMonitor == m_DesiredScreenID && _bFullscreen)
    {
      ...
      g_Log->Info("CDisplayDX::createwindow SetWindowPos x=%u y=%u w=%u h=%u",
                  monitorInfo.rcMonitor.left, monitorInfo.rcMonitor.top,
                  monitorInfo.rcMonitor.right - monitorInfo.rcMonitor.left,
                  monitorInfo.rcMonitor.bottom - monitorInfo.rcMonitor.top);
    }
  ...
  HWND hWnd =
      CreateWindowEx(exStyle, L"ElectricsheepWndClass", L"Electricsheep", style,
...
/*
        Initialize().
        We already have a window from the screensaver, so store it and pass
   along all 0's to initialize a child window.
*/
bool CDisplayDX::Initialize(HWND _hWnd, bool _bPreview)
{
  ...
  if (_bPreview)
  {
    ...
    m_WindowHandle = CreateWindow(
        L"ElectricsheepWndClass", L"Preview", dwStyle, rc.left, rc.top,
        rc.right - rc.left, rc.bottom - rc.top, _hWnd, NULL, hInstance, NULL);
    g_Log->Info("CDisplayDX::Initialize x=%u y=%u w=%u h=%u", rc.left, rc.top,
                rc.right - rc.left, rc.bottom - rc.top);
    if (m_WindowHandle == NULL)
    {
      g_Log->Error(
          "CDisplayDX::Initialize unable to create window for preview");
    ...
    // In preview mode, "pause" (enter a limited message loop) briefly before
    // proceeding, so the display control panel knows to update itself.
    ...
    InvalidateRect(GetParent(_hWnd), NULL,
                   FALSE); // Invalidate the hwnd so it gets drawn
  ...
  else
  {
    ...
    g_Log->Info("CDisplayDX::Initialize right=%u left=%u bottom=%u top=%u",
                rc.right, rc.left, rc.bottom, rc.top);
    // DWORD exstyle = 0;//WS_EX_TOPMOST;
    // DWORD style = WS_CHILD | WS_VISIBLE;

    m_WindowHandle =
        _hWnd; // CreateWindowEx( exstyle, "Electricsheep", "Saver", style, 0,
               // 0, cx, cy, _hWnd, NULL, hInstance, NULL );
    if (m_WindowHandle == NULL)
    {
...
void CDisplayDX::Update()
{
  ...
  /*if( m_bPreview )
  {
          ...
          InvalidateRect( m_WindowHandle, NULL, FALSE );    // Invalidate the
  hwnd so it gets drawn UpdateWindow( m_WindowHandle );
  }*/
