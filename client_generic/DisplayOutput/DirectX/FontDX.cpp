...
bool CFontDX::Create()
{
  ...
  // if (hdc != NULL)
  //{
  //	height = -MulDiv(m_FontDescription.Height(), GetDeviceCaps(hdc
  //,LOGPIXELSY), 72); 	ReleaseDC(NULL, hdc); 	height =
  //-MulDiv(m_FontDescription.Height(), 72, 72) - 1;
  // }
  // else
  //{
  //	height = -MulDiv(m_FontDescription.Height(), 72, 72) - 1;
  // }
  HRESULT hr = g_DLLFun->D3DXCreateFontA_fun(
      m_pDevice, height, 0, fontFlags, 0, m_FontDescription.Italic(),
      DEFAULT_CHARSET, OUT_DEFAULT_PRECIS,
      m_FontDescription.AntiAliased() ? ANTIALIASED_QUALITY
                                      : NONANTIALIASED_QUALITY,
      DEFAULT_PITCH | FF_DONTCARE, m_FontDescription.TypeFace().c_str(),
      &m_pDXFont);
