...
const int32 GetBlendConstant(const int32 _src)
{
  static int32 formatLut[] = {
      ...
      D3DBLEND_INVDESTALPHA, D3DBLEND_SRCALPHASAT,
  };
...
bool CRendererDX::EndFrame(bool drawn = true)
{
  ...
  //{
  //	if (swapchain->Present(NULL, NULL, NULL, NULL, D3DPRESENT_DONOTWAIT) ==
...
void CRendererDX::Apply()
{
  ...
  if (m_spActiveBlend != m_spSelectedBlend)
  {
    ...
    m_pDevice->SetRenderState(D3DRS_ALPHABLENDENABLE,
                              m_spActiveBlend->m_bEnabled);
    m_pDevice->SetRenderState(D3DRS_SRCBLEND,
                              GetBlendConstant(m_spActiveBlend->m_Src));
    m_pDevice->SetRenderState(D3DRS_DESTBLEND,
                              GetBlendConstant(m_spActiveBlend->m_Dst));
    m_pDevice->SetRenderState(D3DRS_BLENDOP,
                              GetBlendMode(m_spActiveBlend->m_Mode));
  }
                       ...
                       const Base::Math::CRect &_rect, uint32 _flags)
{
  ...
  if (_flags & CBaseFont::Bottom)
    d3dFlags |= DT_BOTTOM; // bug if CBaseFont::Bottom == 0 because of check
                           // _flag & 0 != 0
  if (_flags & CBaseFont::Top)
    d3dFlags |= DT_TOP;
  if (_flags & CBaseFont::Center)
    d3dFlags |= DT_CENTER;
  if (_flags & CBaseFont::Left)
    d3dFlags |= DT_LEFT;
  if (_flags & CBaseFont::Right)
    d3dFlags |= DT_RIGHT;
  if (_flags & CBaseFont::VCenter)
    d3dFlags |= DT_VCENTER;
  if (_flags & CBaseFont::NoClip)
    d3dFlags |= DT_NOCLIP;
  if (_flags & CBaseFont::ExpandTabs)
    d3dFlags |= DT_EXPANDTABS;
  if (_flags & CBaseFont::WordBreak)
                           ...
                           const Base::Math::CVector4 &_color, const fp4 _width)
{
  /*	ASSERT( m_pLine != NULL );
          ...
          D3DXVECTOR2 aVertices[] = { D3DXVECTOR2( _rect.m_X0, _rect.m_Y0 ),
                                                                  D3DXVECTOR2(
     _rect.m_X1, _rect.m_Y0 ), D3DXVECTOR2( _rect.m_X1, _rect.m_Y1 ),
                                                                  D3DXVECTOR2(
     _rect.m_X0, _rect.m_Y1 ), D3DXVECTOR2( _rect.m_X0, _rect.m_Y0 )	};

          DWORD rgba = D3DCOLOR_COLORVALUE( _color.m_X, _color.m_Y, _color.m_Z,
     _color.m_W );

          m_pLine->SetWidth( _width );
          m_pLine->Draw( aVertices, sizeof(aVertices) / sizeof(aVertices[0]),
     rgba );*/
}
                           ...
                           const Base::Math::CVector4 &_color)
{
  ...
  if (!SUCCEEDED(
          m_pDevice->DrawPrimitiveUP(D3DPT_TRIANGLEFAN, 2, qv, sizeof(Q))))
  {
                               ...
                               const fp4 _width)
{
    ...
    for (uint32 y = 0; y < 32; y++)
      for (uint32 x = 0; x < 32; x++)
      {
        fp4 c = Base::Math::saturate(
            1.0f - powf(Base::Math::Sqrt(fp4(x * x + y * y)) / 31.0f, 1.0f));
  ...
  Q border[] = {
      Q(x0, y0bw, 1, 0, rgba),   Q(x0, y0, 1, 1, rgba),
      Q(x0bw, y0bw, 0, 0, rgba), Q(x0bw, y0, 0, 1, rgba),
      Q(x1bw, y0bw, 0, 0, rgba), Q(x1bw, y0, 0, 1, rgba),

      Q(x1bw, y0, 0, 1, rgba),   Q(x1, y0, 1, 1, rgba),
      Q(x1bw, y0bw, 0, 0, rgba), Q(x1, y0bw, 1, 0, rgba),
      Q(x1bw, y1bw, 0, 0, rgba), Q(x1, y1bw, 1, 0, rgba),

      Q(x1, y1bw, 1, 0, rgba),   Q(x1, y1, 1, 1, rgba),
      Q(x1bw, y1bw, 0, 0, rgba), Q(x1bw, y1, 0, 1, rgba),
      Q(x0bw, y1bw, 0, 0, rgba), Q(x0bw, y1, 0, 1, rgba),

      Q(x0bw, y1, 0, 1, rgba),   Q(x0, y1, 1, 1, rgba),
      Q(x0bw, y1bw, 0, 0, rgba), Q(x0, y1bw, 1, 0, rgba),
      Q(x0bw, y0bw, 0, 0, rgba), Q(x0, y0bw, 1, 0, rgba),
  };
  ...
  if (!SUCCEEDED(m_pDevice->DrawPrimitiveUP(D3DPT_TRIANGLESTRIP, 22, border,
                                            sizeof(Q))))
  {
                           ...
                           const Base::Math::CVector4 &_color, const fp4 _width)
{
  /*	ASSERT( m_pLine != NULL );
          ...
          D3DXVECTOR2 aVertices[] = { D3DXVECTOR2( _start.m_X, _start.m_Y ),
     D3DXVECTOR2( _end.m_X, _end.m_Y ) };
