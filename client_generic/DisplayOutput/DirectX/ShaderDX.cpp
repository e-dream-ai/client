...
bool CShaderDX::Build(const char *_pVertexShader, const char *_pFragmentShader)
{
  ...
  if (_pVertexShader != NULL)
  {
    ...
    if (g_DLLFun->D3DXCompileShader_fun(
            shaderString.c_str(), (UINT)shaderString.length(), NULL, NULL,
            "main", profile, D3DXSHADER_SKIPVALIDATION, &pShaderBuf,
  ...
  if (_pFragmentShader != NULL)
  {
            ...
            &pErrorsBuf, &m_pFragmentConstants) == D3D_OK)
    {
      if (FAILED(m_pDevice->CreatePixelShader(
              (DWORD *)pShaderBuf->GetBufferPointer(), &m_pFragmentShader)))
      {
  ...
  if (m_pVertexShader)
  {
    ...
    for (uint32 i = 0; i < vsDesc.Constants; i++)
    {
      ...
      m_pVertexConstants->GetConstantDesc(
          m_pVertexConstants->GetConstant(NULL, i), &cDesc, &count);
      ...
      if (cDesc.Class == D3DXPC_MATRIX_COLUMNS)
      {
        ...
        case D3DXPT_FLOAT:
          ...
          break;
        default:
          g_Log->Warning("Unknown vertex uniform type for %s -> %d", cDesc.Name,
                         cDesc.Type);
        }
      }
      else
      ...
      m_Uniforms[cDesc.Name] = new CShaderUniformDX(m_pDevice, cDesc.Name,
                                                    eType, cDesc.RegisterIndex);
    }
  }
  ...
  if (m_pFragmentShader)
  {
    ...
    for (uint32 i = 0; i < psDesc.Constants; i++)
    {
      ...
      m_pFragmentConstants->GetConstantDesc(
          m_pFragmentConstants->GetConstant(NULL, i), &cDesc, &count);

      if (cDesc.Type >= D3DXPT_SAMPLER && cDesc.Type <= D3DXPT_SAMPLERCUBE)
        m_Samplers[cDesc.Name] = new CShaderUniformDX(
        ...
        else
        {
          ...
          if (cDesc.Class == D3DXPC_VECTOR)
          {
            ...
            case D3DXPT_FLOAT:
              ...
              break;
            default:
              g_Log->Warning("Unknown pixel uniform type for %s -> %d",
                             cDesc.Name, cDesc.Type);
            }
          }
          else
          {
...
void CShaderUniformDX::Apply()
{
  ...
  case eUniform_Float:
    m_pDevice->SetPixelShaderConstantF(m_Index, (const float *)m_float4Data, 1);
    break;
  case eUniform_Float4:
  ...
  // Uncommenting this will break shaders after device reset and few other
  // situations
  // m_bDirty = false;
}
