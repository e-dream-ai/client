    ...
    std::vector<std::pair<std::string, eUniformType>> _uniforms)
{
  ...
  id<MTLFunction> fragmentFunc =
      [rendererContext->shaderLibrary newFunctionWithName:@(_pFragmentShader)];
  ASSERT(vertexFunc);
  ASSERT(fragmentFunc);
                              ...
                              const Base::Math::CVector4 &_color)
{
  ...
  @autoreleasepool
  {
    ...
    vector_float3 translation = {_text->GetRect().m_X0, _text->GetRect().m_Y0,
                                 0};
    ...
    matrix_float4x4 projectionMatrix = matrix_orthographic_projection(
        0, kMetalTextReferenceContextSize, 0, kMetalTextReferenceContextSize);
  ...
  @autoreleasepool
  {
    ...
    for (uint32_t i = 0; i < MAX_TEXUNIT; ++i)
    {
      spCTextureFlatMetal selectedTexture =
          static_pointer_cast<CTextureFlatMetal>(m_aspSelectedTextures[i]);
      if (selectedTexture)
      {
    ...
    uniforms.uvRect =
        vector_float4{_uvrect.m_X0, _uvrect.m_Y0, _uvrect.m_X1 - _uvrect.m_X0,
