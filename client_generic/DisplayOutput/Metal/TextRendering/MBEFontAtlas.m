...
- (void)encodeWithCoder:(NSCoder *)aCoder
{
    ...
    [aCoder encodeFloat:(float)self.bottomRightTexCoord.x forKey:MBERightTexCoordKey];
    [aCoder encodeFloat:(float)self.bottomRightTexCoord.y forKey:MBEBottomTexCoordKey];
}
...
- (CGSize)estimatedGlyphSizeForFont:(NSFont *)font
{
    ...
    CGFloat averageGlyphWidth = ceilf((float)exemplarStringSize.width / exemplarString.length);
    ...
    for (size_t y = 1; y < height - 2; ++y)
    {
        for (size_t x = 1; x < width - 2; ++x)
        {
            if (distanceMap[(y - 1) * width + (x - 1)] + distDiag < distance(x, y))
            {
                ...
                distance(x, y) = (float)hypot(x - nearestpt(x, y).x, y - nearestpt(x, y).y);
            }
                      ...
                      normalizationFactor:(float)normalizationFactor
{
    ...
    for (size_t y = 0; y < height; ++y)
    {
        for (size_t x = 0; x < width; ++x)
        {
            ...
            float clampDist = (float)fmax(-normalizationFactor, fmin(dist, normalizationFactor));
