  ...
  if (vsResult && fsResult)
  {
    ...
    if (linkResult)
    {
      ...
      for (int i = 0; i < uniformCount; i++)
      {
        GLenum type;
        GLint length, size;
        glGetActiveUniformARB(m_Program, i, maxLength, &length, &size, &type,
                              name);
...
#ifdef MAC
          m_Samplers[name] =
              new CShaderUniformGL(cgl_ctx, name, eUniform_Sampler, location);
#else
          m_Samplers[name] =
...
#else
              m_Uniforms[name] = new CShaderUniformGL(
                  name, eType, glGetUniformLocationARB(m_Program, name), size);
#endif
            }
