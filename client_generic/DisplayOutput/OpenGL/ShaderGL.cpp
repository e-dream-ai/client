...
bool CShaderGL::Build(const char *_pVertexShader, const char *_pFragmentShader)
{
    ...
    if (GLEE_ARB_shading_language_100)
    {
        ...
        if (bFirst)
        {
// strangely enough on OSX 10.4 GL_ARB_shading_language_100 is defined as 1 but
// GL_SHADING_LANGUAGE_VERSION_ARB is not defined at all
...
#define GL_SHADING_LANGUAGE_VERSION_ARB 0x8B8C
    ...
    if (_pVertexShader != NULL)
    {
        ...
        if (vsResult)
            ...
            infoLogPos +=
                sprintf(infoLog + infoLogPos, "Vertex shader error:\n");

        glGetInfoLogARB(m_VertexShader,
                        sizeof(infoLog) - static_cast<size_t>(infoLogPos), &len,
                        infoLog + infoLogPos);
        infoLogPos += len;
    }
    else
    ...
    if (_pFragmentShader != NULL)
    {
        ...
        glGetObjectParameterivARB(m_FragmentShader,
                                  GL_OBJECT_COMPILE_STATUS_ARB, &fsResult);
        if (fsResult)
            glAttachObjectARB(m_Program, m_FragmentShader);
        else
            infoLogPos +=
                sprintf(infoLog + infoLogPos, "Fragment shader error:\n");

        glGetInfoLogARB(m_FragmentShader,
                        sizeof(infoLog) - static_cast<size_t>(infoLogPos), &len,
                        infoLog + infoLogPos);
        infoLogPos += len;
    }
    ...
    if (vsResult && fsResult)
    {
        ...
        glGetObjectParameterivARB(m_Program, GL_OBJECT_LINK_STATUS_ARB,
                                  &linkResult);
        glGetInfoLogARB(m_Program,
                        sizeof(infoLog) - static_cast<size_t>(infoLogPos), &len,
                        infoLog + infoLogPos);
        // infoLogPos += len;
        ...
        if (linkResult)
        {
            ...
            glGetObjectParameterivARB(
                m_Program, GL_OBJECT_ACTIVE_UNIFORM_MAX_LENGTH_ARB, &maxLength);
            ...
            for (int i = 0; i < uniformCount; i++)
            {
                GLenum type;
                GLint length, size;
                glGetActiveUniformARB(m_Program, i, maxLength, &length, &size,
                                      &type, name);
...
#ifdef MAC
                    m_Samplers[name] = new CShaderUniformGL(
                        cgl_ctx, name, eUniform_Sampler, location);
#else
                    m_Samplers[name] =
                        new CShaderUniformGL(name, eUniform_Sampler, location);
#endif
                    nSamplers++;
                }
                else
                {
                    ...
                    if (strncmp(name, "gl_", 3) != 0)
                    {
                            ...
                            (bracket[1] == '0' && bracket[2] == ']'))
                        {
                            ...
                            {
                            case GL_FLOAT:
                                eType = eUniform_Float;
                                break;
                            case GL_FLOAT_VEC2_ARB:
                                eType = eUniform_Float2;
                                break;
                            case GL_FLOAT_VEC3_ARB:
                                eType = eUniform_Float3;
                                break;
                            case GL_FLOAT_VEC4_ARB:
                                eType = eUniform_Float4;
                                break;
                            case GL_INT:
                                eType = eUniform_Int;
                                break;
                            ...
                            case GL_INT_VEC4_ARB:
                                ...
                                break;
                            case GL_BOOL_ARB:
                                eType = eUniform_Boolean;
                                break;
                            case GL_BOOL_VEC2_ARB:
                                eType = eUniform_Boolean2;
                                break;
                            case GL_BOOL_VEC3_ARB:
                                eType = eUniform_Boolean3;
                                break;
                            case GL_BOOL_VEC4_ARB:
                                eType = eUniform_Boolean4;
                                break;
                            case GL_FLOAT_MAT2_ARB:
                                eType = eUniform_Matrix2;
                                break;
                            case GL_FLOAT_MAT3_ARB:
                                eType = eUniform_Matrix3;
                                break;
                            case GL_FLOAT_MAT4_ARB:
                                eType = eUniform_Matrix4;
                                break;
                            }

#ifdef MAC
                            m_Uniforms[name] = new CShaderUniformGL(
                                cgl_ctx, name, eType,
                                glGetUniformLocationARB(m_Program, name), size);
#else
                            m_Uniforms[name] = new CShaderUniformGL(
                                name, eType,
                                glGetUniformLocationARB(m_Program, name), size);
            ...
            /*			for (int i = 0; i < nUniforms; i++)
                                    {
                                            int constantSize =
               constantTypeSizes[uniforms[i].type] * uniforms[i].nElements;
                                            uniforms[i].data = new
               ubyte[constantSize]; memset(uniforms[i].data, 0, constantSize);
