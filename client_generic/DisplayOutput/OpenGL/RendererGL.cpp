...
void CRendererGL::Verify(const char * /*_file*/, const int32 /*_line*/)
{
  ...
  if( error != GL_NO_ERROR )
  {
          ...
          else if( error ==    GL_INVALID_OPERATION) msg =
  "GL_INVALID_OPERATION"; else if( error ==    GL_STACK_OVERFLOW)	msg =
  "GL_STACK_OVERFLOW"; else if( error ==    GL_STACK_UNDERFLOW)	msg =
  "GL_STACK_UNDERFLOW"; else if( error ==    GL_OUT_OF_MEMORY)	msg =
  "GL_OUT_OF_MEMORY"; else if( error ==    GL_INVALID_FRAMEBUFFER_OPERATION_EXT)
  msg = "GL_INVALID_FRAMEBUFFER_OPERATION_EXT"; else msg = "Unrecognized OpenGL
  error";

          g_Log->Error( "OpenGLError - %s(%d) in %s(%d)", msg.c_str(), error,
  _file, _line );
...
void CRendererGL::Apply()
{
  ...
  if (m_spActiveBlend != m_spSelectedBlend)
  {
    ...
    glEnable(GL_BLEND);
    glBlendFunc(GetBlendConstant(m_spActiveBlend->m_Src),
...
/*
 */
spCTextureFlat CRendererGL::NewTextureFlat(spCImage _spImage,
                                           const uint32 _flags)
{
  SetCurrentGLContext();

#ifdef MAC
  spCTextureFlat spTex = new CTextureFlatGL(
      _flags | ((GetTextureTargetType() == eTexture2DRect) ? kRectTexture : 0),
      cgl_ctx);
#else
  spCTextureFlat spTex = new CTextureFlatGL(
      _flags | ((GetTextureTargetType() == eTexture2DRect) ? kRectTexture : 0));
                       ...
                       const Base::Math::CRect &_rect, uint32 /*_flags*/)
{
  ...
  for (size_t i = 0; i < _text.size(); i++)
  {
    ...
    Base::Math::CRect texr(
        glyph->tex_x1 * texpow2r.Width(), glyph->tex_y1 * texpow2r.Height(),
        glyph->tex_x2 * texpow2r.Width(),
        (glyph->tex_y1 + glFont->TexLineHeight()) * texpow2r.Height());
                                                ...
                                                const std::string &_text)
{
  ...
  return Base::Math::CVector2(textwidth / (fp4)m_spDisplay->Width(),
                              textheight / (fp4)m_spDisplay->Height());
}
                           ...
                           const Base::Math::CRect &_uvrect)
{
  ...
  /*float vertices[]  = {r.m_X0, r.m_Y0, r.m_X1, r.m_Y0, r.m_X1, r.m_Y1, r.m_X0,
  r.m_Y1}; float texcoords[] = {_uvrect.m_X0, _uvrect.m_Y0, _uvrect.m_X1,
  _uvrect.m_Y0, _uvrect.m_X1, _uvrect.m_Y1, _uvrect.m_X0, _uvrect.m_Y1 };
  ...
  if (m_spSoftCorner == NULL)
  {
    ...
    for (uint32 y = 0; y < 32; y++)
      for (uint32 x = 0; x < 32; x++)
      {
        ...
        tmpImage->PutPixel(static_cast<int32>(x), static_cast<int32>(y), c, c,
                           c, c);
      }
