...
CUnixGL::~CUnixGL()
{
    ...
    if (!bScreensaverMode)
        ...
        int dummy = system(
            "xset s on 2>/dev/null; xset +dpms 2>/dev/null; gconftool-2 --set --type bool \
...
bool CUnixGL::Initialize(const uint32 _width, const uint32 _height,
                         const bool _bFullscreen)
{
    ...
    int singleBufferAttributess[] = {
        ...
        GLX_RGBA_BIT,
        GLX_RED_SIZE,
        1, //	Request a single buffered color buffer
        GLX_GREEN_SIZE,
        1, //	with the maximum number of color bits
        GLX_BLUE_SIZE,
    ...
    pFbConfig = glXChooseFBConfig(m_pDisplay, DefaultScreen(m_pDisplay),
                                  doubleBufferAttributes, &numElements);
    if (pFbConfig)
    ...
    {
        pFbConfig = glXChooseFBConfig(m_pDisplay, DefaultScreen(m_pDisplay),
                                      singleBufferAttributess, &numElements);
        if (pFbConfig)
    ...
    winAttributes.colormap =
        XCreateColormap(m_pDisplay, RootWindow(m_pDisplay, pVisualInfo->screen),
    ...
    else
    {
        m_Window = XCreateWindow(
            m_pDisplay, RootWindow(m_pDisplay, pVisualInfo->screen), 0, 0,
            _bFullscreen ? m_WidthFS : m_Width,
            _bFullscreen ? m_HeightFS : m_Height, 0, pVisualInfo->depth,
            InputOutput, pVisualInfo->visual,
            CWBorderPixel | CWColormap | CWEventMask, &winAttributes);
        ...
        m_GlxContext = glXCreateNewContext(m_pDisplay, renderFBConfig,
                                           GLX_RGBA_TYPE, 0, GL_TRUE);
        m_GlxWindow = glXCreateWindow(m_pDisplay, renderFBConfig, m_Window, 0);
        glXMakeContextCurrent(m_pDisplay, m_GlxWindow, m_GlxWindow,
                              m_GlxContext);

        /* disable screensaver and screen blanking in non-screensaver mode */
        int dummy = system(
            "xset s off 2>/dev/null; xset -dpms 2>/dev/null; gconftool-2 --set --type bool \
    ...
    invisibleCursor = XCreatePixmapCursor(m_pDisplay, bitmapNoData,
...
#else
    m_Window =
        XCreateWindow(m_pDisplay, RootWindow(m_pDisplay, pVisualInfo->screen),
                      0, 0, _bFullscreen ? m_WidthFS : m_Width,
                      _bFullscreen ? m_HeightFS : m_Height, 0,
    ...
    if (!bScreensaverMode)
        setFullScreen(_bFullscreen);
    XMapRaised(m_pDisplay, m_Window);
    if (!bScreensaverMode && _bFullscreen)
        XIfEvent(m_pDisplay, &event, WaitForNotify, (XPointer)m_Window);
...
void CUnixGL::setWindowDecorations(bool enabled)
{
    ...
    if (hintsAtom != None)
    {
        ...
        XGetWindowProperty(m_pDisplay, m_Window, hintsAtom, 0,
                           ...
                           AnyPropertyType, &typeAtom, &iFormat, &ulItems,
                           &ulBytesAfter, &pucData);
    ...
    if (!set)
    {
        XSetTransientForHint(m_pDisplay, m_Window,
...
void CUnixGL::setFullScreen(bool enabled)
{
    ...
    if (m_FullScreen)
    {
        ...
        XA_NET_WM_STATE = XInternAtom(m_pDisplay, "_NET_WM_STATE", False);
        XA_NET_WM_STATE_ADD =
            XInternAtom(m_pDisplay, "_NET_WM_STATE_ADD", False);

        XA_NET_WM_STATE_MAXIMIZED_VERT =
            XInternAtom(m_pDisplay, "_NET_WM_STATE_MAXIMIZED_VERT", False);
        XA_NET_WM_STATE_MAXIMIZED_HORZ =
            XInternAtom(m_pDisplay, "_NET_WM_STATE_MAXIMIZED_HORZ", False);
        XA_NET_WM_STATE_FULLSCREEN =
            XInternAtom(m_pDisplay, "_NET_WM_STATE_FULLSCREEN", False);
    ...
    // XChangeWindowAttributes(m_pDisplay, m_Window, CWOverrideRedirect,
    // &winAttrib);
    XMapRaised(m_pDisplay, m_Window);
...
void CUnixGL::alwaysOnTop()
{
    ...
    // XSendEvent(m_pDisplay, m_Window, False, SubstructureRedirectMask,
    // reinterpret_cast<XEvent*>(&xev)); state = XGetAtomName(mDisplay,
    // xev.data.l[1]); mp_msg(MSGT_VO, MSGL_V,
...
/*
 */
void CUnixGL::Update() { checkClientMessages(); }

/*
 */
void CUnixGL::SwapBuffers() { glXSwapBuffers(m_pDisplay, m_GlxWindow); }
...
void CUnixGL::checkClientMessages()
{
    ...
    if (XCheckTypedEvent(m_pDisplay, ClientMessage, &xEvent))
        if (*XGetAtomName(m_pDisplay, xEvent.xclient.message_type) ==
            *"WM_PROTOCOLS")
            m_bClosed = true;
...
/*bool CUnixGL::checkKeyEvent(KeyEvent& event)
{
    ...
    gotEvent = XCheckWindowEvent( m_pDisplay, m_Window, KeyPressMask |
