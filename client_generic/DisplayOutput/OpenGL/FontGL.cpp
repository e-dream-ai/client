...
#define SWAP_SHORT_(x)                                                         \
    ((unsigned short)((unsigned short)(x) << 8) |                              \
     (unsigned short)((unsigned short)(x) >> 8))
#else
...
*/
CFontGL::~CFontGL() { SAFE_DELETE_ARRAY(m_glyphs); }
...
bool CFontGL::Create()
{
    ...
    for (uint32 i = 0; i < n_chars; ++i)
    {
        ...
        m_glyphs[i].tex_x2 =
            static_cast<fp4>(buffer.x + buffer.width) / (fp4)width;
    ...
    if (height != m_spTextImage->GetHeight() ||
...
fp4 CFontGL::LineHeight() const { return m_lineHeight; }

fp4 CFontGL::TexLineHeight() const { return m_texLineHeight; }
...
void CFontGL::Reupload() { m_spTextTexture->Reupload(); }
