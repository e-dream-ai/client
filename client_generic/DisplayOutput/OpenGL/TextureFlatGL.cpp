  ...
  while ((pSrc = _spImage->GetData(mipMapLevel)) != NULL)
  {
    if (format.isCompressed())
    {
      ...
      glCompressedTexImage2DARB(
          ...
          _spImage->GetWidth(mipMapLevel), _spImage->GetHeight(mipMapLevel), 0,
    ...
    else
    {
      ...
      else
      {
        ...
        texHeight =
            Base::Math::UpperPowerOfTwo(_spImage->GetHeight(mipMapLevel));
      }
      ...
      else
      {
        ...
        glTexSubImage2D(
            m_TexTarget, mipMapLevel, 0, 0, _spImage->GetWidth(mipMapLevel),
            _spImage->GetHeight(mipMapLevel), srcFormat, srcType, pSrc);
        ...
        if (mipMapLevel == 0)
        {
          ...
          if (m_TexTarget == GL_TEXTURE_RECTANGLE_ARB)
            ...
            SetRect(Base::Math::CRect((fp4)imgWidth / (fp4)texWidth,
...
{
  g_Log->Error("Creating textures from hardware accelerated sample buffers is "
