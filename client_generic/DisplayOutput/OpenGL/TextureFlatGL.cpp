 ...
 */
#ifdef MAC
CTextureFlatGL::CTextureFlatGL(const uint32 _flags, CGLContextObj glCtx)
    : CTextureFlat(_flags)
#else
...
bool CTextureFlatGL::Upload(spCImage _spImage)
{
    ...
    static const GLenum srcFormats[] = {0, GL_LUMINANCE, GL_LUMINANCE_ALPHA,
                                        GL_RGB,
#ifdef MAC
                                        GL_BGRA
#else
    ...
    if (format.isFloat())
        internalFormat = internalFormats[format.getFormatEnum() -
                                         (eImage_RGBA32F - eImage_I16F)];
    ...
    while ((pSrc = _spImage->GetData(mipMapLevel)) != NULL)
    {
        ...
        {
            // does the glCompressedTexImage2DARB need also power-of-two sized
            // texture???
            glCompressedTexImage2DARB(
                m_TexTarget, mipMapLevel, internalFormat,
                _spImage->GetWidth(mipMapLevel),
                _spImage->GetHeight(mipMapLevel), 0,
                _spImage->getMipMappedSize(mipMapLevel, 1), pSrc);
        ...
        else
        {
            ...
            {
                texWidth = Base::Math::UpperPowerOfTwo(
                    _spImage->GetWidth(mipMapLevel));
                texHeight = Base::Math::UpperPowerOfTwo(
                    _spImage->GetHeight(mipMapLevel));
            }

            if (texWidth == imgWidth && texHeight == imgHeight)
            {
#ifdef MAC
                // glTexParameteri(m_TexTarget, GL_TEXTURE_STORAGE_HINT_APPLE ,
                // GL_STORAGE_SHARED_APPLE);
                glPixelStorei(GL_UNPACK_CLIENT_STORAGE_APPLE, GL_TRUE);
#endif
                glTexImage2D(m_TexTarget, mipMapLevel, internalFormat, texWidth,
                             texHeight, 0, srcFormat, srcType, pSrc);
            ...
            {
                glTexImage2D(m_TexTarget, mipMapLevel, internalFormat, texWidth,
                             texHeight, 0, srcFormat, srcType, NULL);
                glTexSubImage2D(m_TexTarget, mipMapLevel, 0, 0,
                                _spImage->GetWidth(mipMapLevel),
                                _spImage->GetHeight(mipMapLevel), srcFormat,
                                srcType, pSrc);
...
#endif
                        SetRect(
                            Base::Math::CRect((fp4)imgWidth, (fp4)imgHeight));
                    else
                        SetRect(
                            Base::Math::CRect((fp4)imgWidth / (fp4)texWidth,
                                              (fp4)imgHeight / (fp4)texHeight));
                }
            }
...
{
    g_Log->Error(
        "Creating textures from hardware accelerated sample buffers is "
        "not supported on OpenGL");
    return false;
}
} // namespace DisplayOutput
#endif //! USE_METAL
