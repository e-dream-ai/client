...
bool CImage::SaveDDS(const std::string &_fileName)
{
  ...
  DDSHeader header = {
      ...
      m_Format.isCompressed()
          ? DDPF_FOURCC
          : static_cast<unsigned int>((nChannels < 3) ? 0x00020000 : DDPF_RGB) |
                  ...
                  uint32 channels)
{
  ...
  for (uint32 y = 0; y < height; y += 2)
  {
    for (uint32 x = 0; x < width; x += 2)
    {
      ...
      {
        *dest++ =
            ((src[0] + src[xOff] + src[yOff] + src[yOff + xOff]) + 2) >> 2;
        src++;
      }
      src += xOff;
    }
  ...
  else
  {
    ...
    do
    {
               ...
               newFormat.m_Format <= eImage_RGBA16)
      {
        for (uint32 i = 0; i < nDestChannels; i++)
          ((uint16 *)dest)[i] =
              (uint16)(65535 * Base::Math::Clamped(rgba[i], 0.0f, 1.1f) + 0.5f);
      }
  ...
  case eImage_Bicubic:
    ...
    for (y = 0; y < _newHeight; y++)
    {
      ...
      for (x = 0; x < _newWidth; x++)
      {
        ...
        for (k = 0; k < nChannels; k++)
        {
          b = icerp(src[m_Width * nChannels], src[(m_Width + 1) * nChannels],
                    src[(m_Width + 2) * nChannels],
                    src[(m_Width + 3) * nChannels], static_cast<int32>(wX)) &
              0xFF;
          if (sampleY > 0)
