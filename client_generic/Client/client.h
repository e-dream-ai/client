...
public:
  ...
  virtual bool Startup()
  {
    ...
    g_Player().SetMultiDisplayMode((CPlayer::MultiDisplayMode)g_Settings()->Get(
    ...
    //	Get hud font size.
    std::string hudFontName =
        g_Settings()->Get("settings.player.hudFontName", std::string("Lato"));
    ...
    m_HudManager->Add("dreamcredits",
                      std::make_shared<Hud::CStatsConsole>(
                          Base::Math::CRect(1, 1), hudFontName, hudFontSize));
    ...
    spHelpMessage->Add(new Hud::CStringStat(
        ...
        "overlay\n\nby Scott Draves and open source programmers all over the "
        "world",
        ""));
    ...
    else if (g_Settings()->Get("settings.content.download_mode", true) == false)
      spStats->Add(
          new Hud::CTimeCountDownStat("svstat", "", "Downloading disabled"));
    else
      spStats->Add(
          new Hud::CTimeCountDownStat("svstat", "", "Preparing downloader..."));
    ...
    if (displayMode == 2)
      spStats->Add(new Hud::CAverageCounter(
          "displayfps",
          g_Player().Renderer()->Description() + " piecewise cubic display at ",
          " fps", 1.0));
      ...
      spStats->Add(new Hud::CAverageCounter(
          "displayfps", g_Player().Renderer()->Description() + " display at ",
          " fps", 1.0));
      ...
      if ((percent = strchr(m_SplashFilename.c_str(), '%')))
      {
        if (percent[1] == 'd')
        {
          ...
          while (1)
          {
            ...
            snprintf(fNameFormatted, FILENAME_MAX, m_SplashFilename.c_str(),
                     m_nSplashes);
    ...
    // if multiple splashes are found then they are loaded when the timer goes
    // off, not here
    if (m_SplashFilename.empty() == false &&
    ...
    m_spCrossFade = std::make_shared<Hud::CCrossFade>(
        g_Player().Display()->Width(), g_Player().Display()->Height(), true);
      ...
      if ((drawNoSheepIntro && displayUnit == 0) || (drawn && displayUnit == 0))
      {
        ...
        if (m_spSplashPNG)
        {
          if (m_SplashPNGDelayTimer.Time() > m_PNGDelayTimer)
          {
            ...
            // update m_spSplashPNG here, so every time it is shown, it is
            // randomized among our shuffle group.
        ...
        {
          std::string deleted;
          if (ContentDownloader::Shepherd::PopOverflowMessage(deleted) &&
              (deleted != ""))
          {
  ...
  virtual bool HandleOneEvent(DisplayOutput::spCEvent &_event)
  {
    ...
    if (_event->Type() == DisplayOutput::CEvent::Event_KEY)
    {
      ...
      case DisplayOutput::CKeyEvent::KEY_UP:
        if (m_pVoter != NULL && m_pVoter->Vote(g_Player().GetCurrentPlayingID(),
                                               true, voteDelaySeconds))
          m_HudManager->Add("splash_pos", m_spSplashPos,
                            voteDelaySeconds * 0.9f);
        break;
      case DisplayOutput::CKeyEvent::KEY_DOWN:
        if (m_pVoter != NULL && m_pVoter->Vote(g_Player().GetCurrentPlayingID(),
                                               false, voteDelaySeconds))
        {
