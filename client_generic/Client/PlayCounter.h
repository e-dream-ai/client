    ...
    if (iter == m_PlayCounts.end())
    {
...
#ifdef WIN32
      pdata->PlayCountFile = _fsopen(generation_path.string().c_str(), "r+b",
      ...
      if (pdata->PlayCountFile == NULL) // file not found
      {
#ifdef WIN32
        pdata->PlayCountFile =
            _fsopen(generation_path.string().c_str(), "w+b", _SH_DENYWR);
#else
        ...
        if (pdata->PlayCountFile == NULL) // unable to open for writing
        {
          ...
          if (max_sheep != fwrite(pdata->PlayCounts, sizeof(uint16), max_sheep,
                                  pdata->PlayCountFile))
          {
            m_ReadOnly = true;
  ...
  void IncPlayCount(uint32 generation, uint32 id)
  {
        ...
        m_PlayCountDecayZ != 100)
    {
      ...
      for (PlayCountMap::iterator jj = m_PlayCounts.begin();
        ...
        for (size_t ii = 0; ii < max_sheep; ++ii)
        {
          jj->second.PlayCounts[ii] =
              uint16(m_PlayCountDecayZ / 100. * jj->second.PlayCounts[ii]);
