  ...
  public:
            ...
            if (pid != 0)
            {
                ...
                if (hProcess != NULL)
                {
                    ...
                    if (EnumProcessModules(hProcess, hMod, sizeof(hMod),
                                           &dwSize) != 0)
                    {
                        if (GetModuleFileNameExA(hProcess, hMod[0], szFileName,
                                                 sizeof(szFileName)) != 0)
                        {

                            std::string extension = szFileName;
                            // OutputDebugStringA(szFileName);
                            if (extension.find("es.scr") != extension.npos ||
                                extension.find("es.exe") != extension.npos)
                            {
    ...
    CMonoInstance(char *strMutexName)
    {
        ...
        if (m_hMutex != NULL)
        {
                ...
                m_dwLastError) // give some time for snapshot
            {
                ...
                m_hMutex = NULL;
                // OutputDebugStringA("WaitForParentPID()
        ...
        {
            // OutputDebugStringA("assuming m_dwLastError =
            // ERROR_ALREADY_EXISTS");
    ...
    BOOL IsAnotherInstanceRunning()
    {
        return (ERROR_ALREADY_EXISTS == m_dwLastError ||
                ERROR_ACCESS_DENIED == m_dwLastError);
    }
};
