...
protected:
                 ...
                 ContentDecoder::sMetaData &_metadata)
  {
    ...
    if (m_spFrameData != NULL)
    {
          ...
          m_spImageRef->GetHeight() != m_spFrameData->Height())
      {
        ...
        m_spImageRef->Create(m_spFrameData->Width(), m_spFrameData->Height(),
...
#ifdef FRAME_DIAG
      g_Log->Info(
          "Grabbing frame %ld/%ld from %ld (first)...prog - %f, seam - %d",
          _metadata.m_FrameIdx, _metadata.m_MaxFrameIdx, _metadata.m_SheepID,
          _metadata.m_TransitionProgress, _metadata.m_IsSeam);
#endif
      ...
      {
        if (m_spSecondImageRef->GetWidth() != spSecondFrameData->Width() ||
            m_spSecondImageRef->GetHeight() != spSecondFrameData->Height())
        {
          //	Frame differs in size, recreate ref image.
          m_spSecondImageRef->Create(spSecondFrameData->Width(),
        ...
        if (_spSecondTexture != NULL)
        {
          ...
          g_Log->Info(
              "Grabbing frame %ld/%d from %ld (second)...prog - %f, seam - %d",
  ...
  bool UpdateInterframeDelta(const fp8 _fpsCap)
  {
    ...
    //	Accumulated time is longer than the requested framerate, we crossed over
    // to the next frame
    if (m_Acc >= dt)
      bCrossedFrame = true;
...
public:
  CFrameDisplay(DisplayOutput::spCRenderer _spRenderer)
  {
    ...
    m_CurTexMoveDir = 1.;
#ifdef USE_METAL
    m_spShader = m_spRenderer->NewShader("quadPassVertex",
                                         "drawDecodedFrameNoBlendingFragment");
#else
    m_spShader = NULL;
#endif
  }
                      ...
                      ContentDecoder::sMetaData &_metadata)
  {
    ...
    else
    {
      currentalpha = (fp4)Base::Math::Clamped(
          m_LastAlpha + Base::Math::Clamped(m_InterframeDelta / m_FadeCount, 0.,
                                            1. / m_FadeCount),
          0., 1.);
    }
    ...
    if (m_spSecondVideoTexture)
    {
      ...
      m_spRenderer->DrawQuad(
          m_texRect, Base::Math::CVector4(1, 1, 1, currentalpha * transCoef),
          m_spVideoTexture->GetRect());
    }
