...
public:
  CElectricSheep_Mac() : CElectricSheep()
  {
    ...
    *m_proxyPass = 0;
    ...
    OSErr err = FSFindFolder(kUserDomain, kApplicationSupportFolderType, false,
                             &foundRef);
    ...
    if (bundle != NULL)
    {
      ...
      CFStringRef cfver = (CFStringRef)CFBundleGetValueForInfoDictionaryKey(
  ...
  virtual bool Startup()
  {
    ...
    // if m_proxyHost is set, the proxy resolver found one. If not, we should
    // not override preferences.
    if (*m_proxyHost)
    {
      ...
      // for now the resolver doesn't support user/password combination, so we
      // should not touch it so user can set it in prefs. g_Settings()->Set(
      // "settings.content.proxy_username", std::string( (char*)m_proxyUser ) );
      // g_Settings()->Set( "settings.content.proxy_password", std::string(
      // (char*)m_proxyPass ) );
    }
  ...
  virtual int GetACLineStatus()
  {
    ...
    if (srcCnt == 0)
    {
      CFRelease(blob);
      CFRelease(sources);
      return -1; // Could not retrieve battery information.  System may not have
                 // a battery.
    }
