...
#define MAXBUF 4096
    ...
    *lpszResult = (CHAR *)malloc(dwBufSize);
    ...
    //	Check result and type again. If we fail here we must free the memory we
    // allocated...
    if (lResult != ERROR_SUCCESS)
    {
      free(*lpszResult);
    ...
    //	We are not guaranteed a null terminated string from RegQueryValueEx so
...
public:
        ...
        m_bAllowFKey(false), m_pD3D9(NULL)
  {
    ...
    //	Windows spawns a screensaver process with idle priority, which will
  ...
  virtual bool Startup()
  {
    ...
    if (m_ScrMode != eWindowed_AllowMultipleInstances && m_ScrMode != eSaver &&
        m_ScrMode != eConfig && m_ScrMode != eFullScreenStandalone)
    {
    ...
    //	Get the application installation directory from the registry. (stored by
    // the installer)
    m_WorkingDir = ".\\";
    ...
    //	If the exe is renamed to .scr, the path lacks trailing slashes for some
    // bizarre reason...
    ...
    if (m_ScrMode != eWindowed &&
        ...
        g_Settings()->Get("settings.player.MultiDisplayMode", 0) !=
            CPlayer::kMDSingleScreen)
      for (DWORD dw = (monnum + 1); dw < g_Player().Display()->GetNumMonitors();
           ++dw)
      {
    ...
    if (_event->Type() == DisplayOutput::CEvent::Event_Power)
    {
      ...
      if (_event->Type() == DisplayOutput::CEvent::Event_KEY)
      {
        ...
        case DisplayOutput::CKeyEvent::KEY_Esc:
          if (m_ScrMode !=
              eFullScreenStandalone) // esc exits windowed and screensaver mode
            return false;
          else
          {
        ...
        if (_event->Type() == DisplayOutput::CEvent::Event_Mouse)
        {
          ...
          if (m_ScrMode != eWindowed &&
              m_ScrMode != eWindowed_AllowMultipleInstances &&
              m_ScrMode != ePreview && m_ScrMode != eFullScreenStandalone)
          {
            ...
            if (spMouse->m_Code == DisplayOutput::CMouseEvent::Mouse_MOVE)
            {
              ...
              if (!m_bMouseUnknown)
              {
                ...
                if (dist > 20 * 20)
                {
                  g_Log->Info("Mouse moved too far(%f), closing...",
                              Base::Math::Sqrt(dist));
                  return false;
                }
              }
