...
void CLog::Attach(const std::string &_location, const uint32 /*_level*/)
{
  ...
  /*
     ...
     sizeof(luaLogging), "logging.lua" ) == 0 ) lua_pcall( m_pState->GetState(),
     0, 0, 0 );
     ...
     '->['.._level..']:'.._func .. '('..tostring(_line)..'): '.. tostring(_msg)
     ) end" ); m_pState->Pop( Script::Call( m_pState->GetState(),
     "createLogFile", "s", _location.c_str() ) );
  */
  m_bActive = true;
}
