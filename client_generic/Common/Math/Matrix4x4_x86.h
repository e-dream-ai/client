...
//	Identity
static const float g_matrix4x4_x86_ident[16] = {
...
class CMatrix4x4_x86
{
  public:
    ...
    CMatrix4x4_x86(float _m11, float _m12, float _m13, float _m14, float _m21,
    ...
    void PerspectiveLH(const float _fovY, const float _aspect, const float _zn,
                       const float _zf);
    void PerspectiveOffCenterLH(const float _minX, const float _maxX,
                                const float _minY, const float _maxY,
                                const float _zn, const float _zf);
    void OrthographicLH(const float _w, const float _h, const float _zn,
                        const float _zf);

    //	Righthanded.
    void AimRH(const CVector3_x86& _to, const CVector3_x86& _up);
    void PerspectiveRH(const float _fovY, const float _aspect, const float _zn,
                       const float _zf);
    void PerspectiveOffCenterRH(const float _minX, const float _maxX,
                                const float _minY, const float _maxY,
                                const float _zn, const float _zf);
    void OrthographicRH(const float _w, const float _h, const float _zn,
                        const float _zf);
...
{
    memcpy(&(m_Mat[0][0]), g_matrix3x3_x86_ident,
           sizeof(g_matrix3x3_x86_ident));
}
...
{
    memcpy(m_Mat, &(_m1.m_Mat[0][0]), 16 * sizeof(float));
}

/*
 */
inline CMatrix4x4_x86::CMatrix4x4_x86(float _m11, float _m12, float _m13,
                                      float _m14, float _m21, float _m22,
                                      float _m23, float _m24, float _m31,
                                      float _m32, float _m33, float _m34,
...
/*
 */
inline void CMatrix4x4_x86::Set(float _m11, float _m12, float _m13, float _m14,
...
{
    memcpy(&(m_Mat[0][0]), g_matrix4x4_x86_ident,
...
/*
 */
inline float CMatrix4x4_x86::Determinant(void)
{
...
/*
   ...
   translation (eg. everything that has [0,0,0,1] as the rightmost column) MUCH
   cheaper then a real 4x4 inversion.
*/
inline void CMatrix4x4_x86::InvertFast(void)
{
    ...
    Set(s * ((M22 * M33) - (M23 * M32)), s * ((M32 * M13) - (M33 * M12)),
        s * ((M12 * M23) - (M13 * M22)), 0.0f, s * ((M23 * M31) - (M21 * M33)),
        s * ((M33 * M11) - (M31 * M13)), s * ((M13 * M21) - (M11 * M23)), 0.0f,
...
inline void CMatrix4x4_x86::MultiplyFast(const CMatrix4x4_x86& _m1)
{
    for (uint32_t i = 0; i < 4; i++)
    {
        ...
        m_Mat[i][1] = mi0 * _m1.m_Mat[0][1] + mi1 * _m1.m_Mat[1][1] +
                      mi2 * _m1.m_Mat[2][1];
        m_Mat[i][2] = mi0 * _m1.m_Mat[0][2] + mi1 * _m1.m_Mat[1][2] +
                      mi2 * _m1.m_Mat[2][2];
    }
...
{
    float d = 1.0f / (M14 * _v.m_X + M24 * _v.m_Y + M34 * _v.m_Z + M44);
    return (
        CVector3_x86((M11 * _v.m_X + M21 * _v.m_Y + M31 * _v.m_Z + M41) * d,
...
/*
 */
inline void CMatrix4x4_x86::Rotate_X(const float _a)
{
    float s, c;
    SinCos(_a, &s, &c);
...
/*
 */
inline void CMatrix4x4_x86::Rotate_Y(const float _a)
{
    float s, c;
    SinCos(_a, &s, &c);
...
/*
 */
inline void CMatrix4x4_x86::Rotate_Z(const float _a)
{
    float s, c;
    SinCos(_a, &s, &c);
...
/*
 */
inline void CMatrix4x4_x86::PerspectiveLH(const float _fovY,
                                          const float _aspect, const float _zn,
                                          const float _zf)
{
    float h = 1.0f / tanf(_fovY * 0.5f);
    float w = h / _aspect;
...
/*
 */
inline void CMatrix4x4_x86::PerspectiveRH(const float _fovY,
                                          const float _aspect, const float _zn,
                                          const float _zf)
{
    float h = 1.0f / tanf(_fovY * 0.5f);
 ...
 */
inline void
CMatrix4x4_x86::PerspectiveOffCenterLH(const float _minX, const float _maxX,
                                       const float _minY, const float _maxY,
                                       const float _zn, const float _zf)
{
    M11 = 2.0f * _zn / (_maxX - _minX);
    M12 = 0.0f, M13 = 0.0f;
    M14 = 0.0f;
    M21 = 0.0f;
    M22 = 2.0f * _zn / (_maxY - _minY);
    M23 = 0.0f;
    M24 = 0.0f;
    M31 = (_minX + _maxX) / (_minX - _maxX);
    M32 = (_maxY + _minY) / (_minY - _maxY);
    M33 = _zf / (_zf - _zn);
    M34 = 1.0f;
    M41 = 0.0f;
    M42 = 0.0f;
 ...
 */
inline void
CMatrix4x4_x86::PerspectiveOffCenterRH(const float _minX, const float _maxX,
                                       const float _minY, const float _maxY,
                                       const float _zn, const float _zf)
{
    M11 = 2.0f * _zn / (_maxX - _minX);
    ...
    M23 = 0.0f;
    M24 = 0.0f;
    M31 = (_minX + _maxX) / (_maxX - _minX);
    M32 = (_maxY + _minY) / (_maxY - _minY);
    M33 = _zf / (_zn - _zf);
    M34 = -1.0f;
    M41 = 0.0f;
    M42 = 0.0f;
...
/*
 */
inline void CMatrix4x4_x86::OrthographicLH(const float _w, const float _h,
                                           const float _zn, const float _zf)
{
    M11 = 2.0f / _w;
    M12 = 0.0f;
...
/*
 */
inline void CMatrix4x4_x86::OrthographicRH(const float _w, const float _h,
                                           const float _zn, const float _zf)
{
    M11 = 2.0f / _w;
    M12 = 0.0f;
...
inline void CMatrix4x4_x86::operator*=(const CMatrix4x4_x86& _m1)
{
    for (uint32_t i = 0; i < 4; i++)
    {
        ...
        m_Mat[i][0] = mi0 * _m1.m_Mat[0][0] + mi1 * _m1.m_Mat[1][0] +
                      mi2 * _m1.m_Mat[2][0] + mi3 * _m1.m_Mat[3][0];
        m_Mat[i][1] = mi0 * _m1.m_Mat[0][1] + mi1 * _m1.m_Mat[1][1] +
                      mi2 * _m1.m_Mat[2][1] + mi3 * _m1.m_Mat[3][1];
        m_Mat[i][2] = mi0 * _m1.m_Mat[0][2] + mi1 * _m1.m_Mat[1][2] +
                      mi2 * _m1.m_Mat[2][2] + mi3 * _m1.m_Mat[3][2];
        m_Mat[i][3] = mi0 * _m1.m_Mat[0][3] + mi1 * _m1.m_Mat[1][3] +
                      mi2 * _m1.m_Mat[2][3] + mi3 * _m1.m_Mat[3][3];
    }
}

/*
 */
inline void CMatrix4x4_x86::Rotate(const CVector3_x86& _vec, const float _a)
{
    CVector3_x86 v(_vec);
    v.NormalizeFast();
                                     ...
                                     CVector4_x86& _dst) const
{
    ...
    _dst.m_Y =
        M12 * _src.m_X + M22 * _src.m_Y + M32 * _src.m_Z + M42 * _src.m_W;
    _dst.m_Z =
        M13 * _src.m_X + M23 * _src.m_Y + M33 * _src.m_Z + M43 * _src.m_W;
    _dst.m_W =
        M14 * _src.m_X + M24 * _src.m_Y + M34 * _src.m_Z + M44 * _src.m_W;
}
                                       ...
                                       const CMatrix4x4_x86& _m1)
{
    CMatrix4x4_x86 m2(
        ...
        _m0.m_Mat[0][0] * _m1.m_Mat[0][0] + _m0.m_Mat[0][1] * _m1.m_Mat[1][0] +
            _m0.m_Mat[0][2] * _m1.m_Mat[2][0] +
            _m0.m_Mat[0][3] * _m1.m_Mat[3][0],
        _m0.m_Mat[0][0] * _m1.m_Mat[0][1] + _m0.m_Mat[0][1] * _m1.m_Mat[1][1] +
            _m0.m_Mat[0][2] * _m1.m_Mat[2][1] +
            _m0.m_Mat[0][3] * _m1.m_Mat[3][1],
        _m0.m_Mat[0][0] * _m1.m_Mat[0][2] + _m0.m_Mat[0][1] * _m1.m_Mat[1][2] +
            _m0.m_Mat[0][2] * _m1.m_Mat[2][2] +
            _m0.m_Mat[0][3] * _m1.m_Mat[3][2],
        _m0.m_Mat[0][0] * _m1.m_Mat[0][3] + _m0.m_Mat[0][1] * _m1.m_Mat[1][3] +
            _m0.m_Mat[0][2] * _m1.m_Mat[2][3] +
            _m0.m_Mat[0][3] * _m1.m_Mat[3][3],

        _m0.m_Mat[1][0] * _m1.m_Mat[0][0] + _m0.m_Mat[1][1] * _m1.m_Mat[1][0] +
            _m0.m_Mat[1][2] * _m1.m_Mat[2][0] +
            _m0.m_Mat[1][3] * _m1.m_Mat[3][0],
        _m0.m_Mat[1][0] * _m1.m_Mat[0][1] + _m0.m_Mat[1][1] * _m1.m_Mat[1][1] +
            _m0.m_Mat[1][2] * _m1.m_Mat[2][1] +
            _m0.m_Mat[1][3] * _m1.m_Mat[3][1],
        _m0.m_Mat[1][0] * _m1.m_Mat[0][2] + _m0.m_Mat[1][1] * _m1.m_Mat[1][2] +
            _m0.m_Mat[1][2] * _m1.m_Mat[2][2] +
            _m0.m_Mat[1][3] * _m1.m_Mat[3][2],
        _m0.m_Mat[1][0] * _m1.m_Mat[0][3] + _m0.m_Mat[1][1] * _m1.m_Mat[1][3] +
            _m0.m_Mat[1][2] * _m1.m_Mat[2][3] +
            _m0.m_Mat[1][3] * _m1.m_Mat[3][3],

        _m0.m_Mat[2][0] * _m1.m_Mat[0][0] + _m0.m_Mat[2][1] * _m1.m_Mat[1][0] +
            _m0.m_Mat[2][2] * _m1.m_Mat[2][0] +
            _m0.m_Mat[2][3] * _m1.m_Mat[3][0],
        _m0.m_Mat[2][0] * _m1.m_Mat[0][1] + _m0.m_Mat[2][1] * _m1.m_Mat[1][1] +
            _m0.m_Mat[2][2] * _m1.m_Mat[2][1] +
            _m0.m_Mat[2][3] * _m1.m_Mat[3][1],
        _m0.m_Mat[2][0] * _m1.m_Mat[0][2] + _m0.m_Mat[2][1] * _m1.m_Mat[1][2] +
            _m0.m_Mat[2][2] * _m1.m_Mat[2][2] +
            _m0.m_Mat[2][3] * _m1.m_Mat[3][2],
        _m0.m_Mat[2][0] * _m1.m_Mat[0][3] + _m0.m_Mat[2][1] * _m1.m_Mat[1][3] +
            _m0.m_Mat[2][2] * _m1.m_Mat[2][3] +
            _m0.m_Mat[2][3] * _m1.m_Mat[3][3],

        _m0.m_Mat[3][0] * _m1.m_Mat[0][0] + _m0.m_Mat[3][1] * _m1.m_Mat[1][0] +
            _m0.m_Mat[3][2] * _m1.m_Mat[2][0] +
            _m0.m_Mat[3][3] * _m1.m_Mat[3][0],
        _m0.m_Mat[3][0] * _m1.m_Mat[0][1] + _m0.m_Mat[3][1] * _m1.m_Mat[1][1] +
            _m0.m_Mat[3][2] * _m1.m_Mat[2][1] +
            _m0.m_Mat[3][3] * _m1.m_Mat[3][1],
        _m0.m_Mat[3][0] * _m1.m_Mat[0][2] + _m0.m_Mat[3][1] * _m1.m_Mat[1][2] +
            _m0.m_Mat[3][2] * _m1.m_Mat[2][2] +
            _m0.m_Mat[3][3] * _m1.m_Mat[3][2],
        _m0.m_Mat[3][0] * _m1.m_Mat[0][3] + _m0.m_Mat[3][1] * _m1.m_Mat[1][3] +
            _m0.m_Mat[3][2] * _m1.m_Mat[2][3] +
            _m0.m_Mat[3][3] * _m1.m_Mat[3][3]);
