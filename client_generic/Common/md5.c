...
#define READ(p)                                                                \
    ((md5_uint32_t)(((md5_uint32_t)((unsigned char*)(p))[0]) |                 \
                    ((md5_uint32_t)((unsigned char*)(p))[1]) << 8 |            \
                    ((md5_uint32_t)((unsigned char*)(p))[2]) << 16 |           \
                    ((md5_uint32_t)((unsigned char*)(p))[3]) << 24))
...
void md5_process_bytes(const void* buffer, size_t len, struct md5_ctx* ctx)
{
    ...
    if (ctx->buflen != 0)
    {
        ...
        add -= add % sizeof(md5_uint32_t);

        memcpy(&ctx->buffer[left_over], buffer, add);
        ctx->buflen += (md5_uint32_t)add;
    ...
    if (len > 0)
    {
        ...
        ctx->buflen = (md5_uint32_t)left_over;
    }
}
...
void md5_process_block(const void* buffer, size_t len, struct md5_ctx* ctx)
{
    ...
    const md5_uint32_t* words = (const md5_uint32_t*)buffer;
    size_t nwords = len / sizeof(md5_uint32_t);
    const md5_uint32_t* endp = words + nwords;
    md5_uint32_t A = ctx->A;
    md5_uint32_t B = ctx->B;
    md5_uint32_t C = ctx->C;
    md5_uint32_t D = ctx->D;

    /* First increment the byte count.  RFC 1321 specifies the possible
       length of the file up to 2^64 bits.  Here we only compute the
       number of bytes.  Do a double word increment.  */
    ctx->total[0] += (md5_uint32_t)len;
    if (ctx->total[0] < len)
        ++ctx->total[1];
