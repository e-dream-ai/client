/*
        ...
        Reference counted smart pointer with garbage collection and/or object
   level thread synchronization.

        SmartPtr			-	The SmartPtr class
        CRefCountPtr		-	Reference Counting Garbage Collection
        CSyncPtr			-	Synchronized access without
   Reference Counting Garbage Collection CSyncRefCountPtr	-
   Synchronized access with Reference Counting Garbage Collection CSmartPtrBase
   -	The base of all above classes. Used as a common base so an assignment
   between different pointers is able.

        Examples on how to use these classes.

        1. Reference Counting.	(CRefCountPtr)
        2. Object Level Thread Synchronization. (CSyncPtr)
        3. Object Level Thread Synchronization & Reference Counting.
   (CSyncRefCountPtr)
        ...
        }	//	Here the object pointed by p2 WILL BE destroyed
   automatically
        ...
        }	//	Here the object pointed by p2 will NOT be destroyed
   automatically
   ...
   automatically if p in ThreadFunc has already released the object.

        void	ThreadFunc( LPSOMETHING p )
        {
                p->do();	//	Synchronized with the other thread
        }//	Here the object pointed by p WILL BE destroyed automatically if
   p2 in TestFunc has already released the object.
   ...
   representation objects. So you will not have nor proper grabage collection
   nor proper thread synchronization.

                if( p1 != p2 )			The comparison will say these
   point to different objects even you think it is the same.
                {
                        ASSERT( FALSE );
                }
...
#ifdef WIN32
//	If you wrap a non-class with the SmartPtr class, you will receive this
...
//
template <class T>
CRefCountRep<T>::CRefCountRep(const T* ptr) : m_pRealPtr((T*)ptr), m_counter(0)
{
}
...
//	Object of type ACCESS (CSyncAccess<T>) will be automatically created on
// the stack.
...
//
template <class T>
CSyncRefCountRep<T>::CSyncRefCountRep(const T* ptr) : CSyncAccessRep<T>(ptr)
{
}
...
/*
        CSyncAccess.
        The SyncAccess class. It is used as an intermediary to achieve object
   Level Thread Synchronization.
*/
template <class T> class CSyncAccess
{
...
*/
template <class T, class REP = CSyncRefCountRep<T>,
...
/*
        ...
        Example:
                ...
                spCRenderer		m_spRenderer;		//	"smart"
   + "pointer" + class tpCRenderer		m_tpRenderer;		//
   "thread" + "pointer" + class tspCRenderer	m_tspRenderer;		//
