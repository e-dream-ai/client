...
static void RecreateProcess(std::string parameters)
{
  ...
  strcpy(
      temp,
      std::string(winfolder + std::string("\\es.scr ") + parameters).c_str());
...
public:
  CProcessForker(const char *_pProcessName, bool _bTerminate = true,
                 const char *_wd = NULL)
      : m_bWaiting(false), m_pAppName(_pProcessName), m_bTerminate(_bTerminate),
        m_pAppDir(_wd)
  {
    m_pEnv = NULL;
    m_pParams = NULL;
  ...
  void Execute()
  {
                        ...
                        (LPVOID)m_pEnv, (CHAR *)m_pAppDir, &si, &pi))
    {
      ...
      FormatMessageA(FORMAT_MESSAGE_FROM_SYSTEM | FORMAT_MESSAGE_IGNORE_INSERTS,
                     0, err, MAKELANGID(LANG_NEUTRAL, SUBLANG_DEFAULT), msg,
                     MAX_PATH, 0);
  ...
  int32 Wait()
  {
    ...
    while (!bExitWhile)
    {
      ...
      if (m_hProcess == INVALID_HANDLE_VALUE)
        ...
        rc = MsgWaitForMultipleObjects(1, &m_hProcess, FALSE, INFINITE,
                                       QS_ALLINPUT);
      switch (rc)
      {
      case WAIT_FAILED:
      ...
      case WAIT_OBJECT_0 + 1:
        ...
        //	Remove any messages that may be in the queue. If the queue
...
public:
  ...
  void Execute()
  {
    ...
    else
    {
      ...
      19); #else setpriority(PRIO_PROCESS, pid, 19); #endif*/
    }
  }
