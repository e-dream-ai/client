...
/*
        CLinkPool.
        When allocation fails, the pool will grow according to TGrow, and try
   the allocation a second time. (4 seems reasonable... )
*/
template <size_t _size, size_t TGrow = 4> class CLinkPool : public CPoolBase
{
    ...
    CLinkPool() : m_bSingletonActive(true)
    {
        if (_size < sizeof(CChunk))
            g_Log->Info("Pool<%d> created with %d bytes overhead!",
                        sizeof(CChunk) - _size);
        else
    ...
    static size_t Pad(size_t _wantedSize)
    {
        return _wantedSize >= sizeof(CChunk) ? _wantedSize : sizeof(CChunk);
    }
  ...
  public:
    ...
    {
        g_Log->Info("Reserving %d objects for pool of size %d...", _number,
                    _size);
    ...
    static CLinkPool& Instance()
    {
        ...
        // printf( "Trying to access shutdown singleton
        // Memory::CLinkPool<%d>\n", _size );
